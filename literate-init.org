...
версию сервера, который был неработоспособен с современными
gradle-проектами и тем самым убил кучу моего времени при настройке.
...
сохранил архив в этот репозиторий. Его, соответственно, надо будет
...
Делаем установку. Сервер качнул [[https://www.eclipse.org/downloads/download.php?file=/jdtls/snapshots/jdt-language-server-1.49.0-202507102148.tar.gz][отсюда]], согласно [[https://github.com/eclipse-jdtls/eclipse.jdt.ls][инструкции]] (из
[[https://github.com/eclipse-jdtls/eclipse.jdt.ls][репозитория]] не собрался). Поддержку Lombok облегчаем до минимума,
иначе он вешает редактор.
...
Регистрируем созданный LSP-сервер с высшим (первым) приоритетом.
...
(add-to-list 'lsp-enabled-clients 'myjdkls)
#+end_src

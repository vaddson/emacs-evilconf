...
(my/leader-def nil 'override
               ...
               "TAB ." '(persp-frame-switch :which-key "Switch or create workspace")
...
Поиск фрейма по имени. Результат - найденный фрейма с указанным именем или nil.
...
Теперь сама функция. Она принимает на вход имя буфера и возвращает
созданный буфер.  Имя буфера не должно содержать символов,
недопустимых для имени файла.

/TODO: Доделать валидацию для имени файла./

#+begin_src emacs-lisp :tangle yes
(defun my/scratch-buffer-create (name)
...
(defun my/make-scratch-main ()
    (interactive)
    (let* ((name "main")
           (buffer (my/scratch-buffer-create name)))
        (pop-to-buffer buffer)
        buffer))
#+end_src

И создание scratch-буфера для текущего рабочего пространства.

#+begin_src emacs-lisp :tangle yes
(defun my/make-scratch-perspective ()
    (interactive)
    (let* ((name (my/--get-current-persp-name))
           (buffer (my/scratch-buffer-create name)))
        (pop-to-buffer buffer)
        buffer))
#+end_src
...
(my/leader-def nil 'override
    ...
    "p x" '(my/make-scratch-perspective :which-key "Scratch workspace"))
#+end_src

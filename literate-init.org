...
(my/leader-def nil 'override
    ...
    "w p r" '(jump-to-register :which-key "Restore from register"))
#+end_src
...
| Change model                       | C-c m   | SPC l m |
| Cancel request                     | C-c k   | SPC l k |
...
/Пока не работает поддержка Spring Boot./

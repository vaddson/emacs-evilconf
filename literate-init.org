...
(более лучшая, чем hilight-indent-guides-mode).
...
Для консолек исключим автодополнения в случае shell (мешается при
удаленной работе). Остальное оставим, как есть. По-умолчанию было так:
: (sh-mode fish-mode shell-mode eshell-mode)

#+begin_src emacs-lisp :tangle yes
(use-package company-shell
    :ensure t
    :config
    (delete 'shell-mode company-shell-modes))
#+end_src

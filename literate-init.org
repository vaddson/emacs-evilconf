...
- [[#программирование][Программирование]]
  ...
  - [[#groovy][Groovy]]
  - [[#kotlin][Kotlin]]
  - [[#java-custom-lsp-server][Java (custom LSP-server)]]
...
(my/leader-def nil 'override
               ...
               "c l w" '(lsp-workspace-folders-remove :which-key "Remove workspace")
...
Ссылки и пакеты:
- typst-ts-mode
- GitHub: [[https://github.com/remimimimimi/org-typst-preview.el][org-typst-preview]]
...
(defun my/make-run-lsp-java-command ()
    ...
    (list "python"
          ...
          (concat "-configuration "
                  (file-name-concat lsp-dir "config_linux"))
          ;; For Lombok:
          ...
          ;; Data directory:
          (concat "-data "
                  (file-name-concat data-dir (project-name (project-current)))))))
#+end_src
...
Запущенный Emacs-демоном, jdtls требует больше ресурсов и быстро
вешает редактор. Поэтому облегчаем работу сервера через выключение
части функций.
...
/Этот раздел закомментирован, поскольку предыдущий ему замена./
...
(use-package lsp-java
    ...
    (add-to-list 'lsp-java-vmargs "-Djdt.ls.lombokSupport.enabled=true")
    ;; (add-to-list 'lsp-java-vmargs "-Declipse.application=org.eclipse.jdt.ls.core.id1")
    ;; (add-to-list 'lsp-java-vmargs "-Dosgi.bundles.defaultStartLevel=4")
    ;; (add-to-list 'lsp-java-vmargs "-Declipse.product=org.eclipse.jdt.ls.core.product")
    ;; (add-to-list 'lsp-java-vmargs "-Dlog.level=ALL")
    ;; (add-to-list 'lsp-java-vmargs "--add-modules=ALL-SYSTEM")
    ;; (add-to-list 'lsp-java-vmargs "--add-opens=java.base/java.util=ALL-UNNAMED")
    ;; (add-to-list 'lsp-java-vmargs "--add-opens=java.base/java.lang=ALL-UNNAMED")
    ;; :init
    ;; (setq lsp-java-java-path "/usr/lib/jvm/java-21-openjdk/bin/java"
    ;;       lsp-java-import-gradle-java-home "/usr/lib/jvm/java-21-openjdk/bin/java"

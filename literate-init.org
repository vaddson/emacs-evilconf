...
#+begin_src emacs-lisp :tangle yes
(setq my/gptel-def-request-params '(:temperature 0.1))
#+end_src
...
(gptel-make-deepseek "DeepSeek"      ;; Any name you want
    ...
    :key (lambda ()                  ;; can be a function that returns the key
             my/deepseek-api-key)
...
(defun my/gptel--session-id ()
    (unless (boundp 'my/gptel--session-id)
        (setq-local my/gptel--session-id (org-id-uuid)))
    my/gptel--session-id)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(setq gptel-use-curl nil  ;; Only for GigaChat.
      gptel-model 'GigaChat-2-Max
      gptel-log-level 'info  ;; 'debug
      gptel-backend (gptel-make-gigachat
                        "GigaChat"
                        :stream t

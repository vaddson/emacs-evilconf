...
Конфигурация Emacs для работы и прочих других дел. Основная эргономика
повторяет Doom. Есть переключение с раскладки по умолчанию Vim (EVIL)
на стандартную раскладку Emacs.
...
- [[#отладка][Отладка]]
- [[#customize-в-отдельном-файле][Customize в отдельном файле]]
...
- [[#среда][Среда]]
  ...
  - [[#безопасность-сетевого-доступа][Безопасность сетевого доступа]]
  - [[#подтверждение-удаления-процессов-при-закрытии-редактора][Подтверждение удаления процессов при закрытии редактора]]
  - [[#пользовательские-переменные-окружения-env-exec-path-from-shell][Пользовательские переменные окружения (env, exec-path-from-shell)]]
- [[#evil-режим-редактирования][Evil. Режим редактирования]]
  ...
  - [[#прокрутка-строки-к-верху-окна][Прокрутка строки к верху окна]]
- [[#внешний-вид-ui-и-gui][Внешний вид UI и GUI]]
  - [[#автоматическая-установка-шрифтов-под-монитор][Автоматическая установка шрифтов под монитор]]
...
- [[#подсказки-выпадашки-автодополнения][Подсказки, выпадашки, автодополнения]]
  - [[#flycheck-проверка-ошибок][Flycheck. Проверка ошибок]]
  - [[#vertico--минималистичный-интерфейс-завершения][Vertico — минималистичный интерфейс завершения]]
  - [[#подсказки-на-клавиши][Подсказки на клавиши]]
  - [[#consult--расширенные-команды-завершения][Consult — расширенные команды завершения]]
  - [[#company-автодополнения][Company. Автодополнения]]
  - [[#marginalia--аннотации-кандидатов][Marginalia — аннотации кандидатов]]
  - [[#prescient--сортировка-и-фильтрация-по-частоте-использования][Prescient — сортировка и фильтрация по частоте использования]]
  - [[#дополнительные-пакеты][Дополнительные пакеты]]
  - [[#embark--контекстное-меню-действий][Embark — контекстное меню действий]]
  - [[#клавиши-перемещения-по-минибуферу][Клавиши перемещения по минибуферу]]
- [[#поиск-и-выделение][Поиск и выделение]]
  - [[#consult-line-живой-поиск-текста][Consult-line. Живой поиск текста]]
...
- [[#работа-с-окнами][Работа с окнами]]
  ...
  - [[#запоминаем-и-восстанавливаем-расположение-окон][Запоминаем и восстанавливаем расположение окон]]
  - [[#максимизация-и-восстановление-прежнего-положения-окон][Максимизация и восстановление прежнего положения окон]]
...
- [[#org][Org]]
  ...
  - [[#сквозные-идентификаторы-в-org-id-и-custom_id][Сквозные идентификаторы в org (ID и CUSTOM_ID)]]
  - [[#клавиши-для-работы-в-org-mode][Клавиши для работы в org-mode]]
  - [[#новый-пункт-в-списке][Новый пункт в списке]]
  ...
  - [[#разнообразная-реакция-на-enter-ret-dwim][Разнообразная реакция на Enter (RET, dwim)]]
  - [[#разнообразная-реакция-на-двойное-нажатие-мышки-mouse-dwim][Разнообразная реакция на двойное нажатие мышки (mouse, dwim)]]
  - [[#открытие-org-ссылки-в-том-же-окне-и-в-окне-соседнем][Открытие org-ссылки в том же окне и в окне соседнем]]
  ...
  - [[#org-transclusion-показ-цитат-из-других-источников-без-их-копирования][Org-transclusion. Показ цитат из других источников без их копирования]]
  - [[#унификация-шрифтов-в-заголовках][Унификация шрифтов в заголовках]]
- [[#org-roam][Org-roam]]
  - [[#установка][Установка]]
...
- [[#инструменты][Инструменты]]
  - [[#ielm-repl-для-emacs-lisp][Ielm. REPL для emacs-lisp]]
  - [[#eww-встроенный-в-emacs-браузер][Eww. Встроенный в Emacs браузер]]
  - [[#treemacs][Treemacs]]
  - [[#magit][Magit]]
...
- [[#инструменты-свои][Инструменты свои]]
  ...
  - [[#читалка-в-консоли-rag][Читалка в консоли (RAG)]]
...
- [[#llm-всевозможные-клиенты][LLM. Всевозможные клиенты]]
  ...
  - [[#gptel-выбор-модели-заданного-бекэнда][GPTel. Выбор модели заданного бекэнда]]
  ...
  - [[#gptel-подключаем-gigachat][GPTel. Подключаем GigaChat]]
    ...
    - [[#настройки][Настройки]]
  - [[#proxy-в-api-gigachat][Proxy в API Gigachat]]
  - [[#работа-с-кодом-автодополнение-оптимизация-и-прочее][Работа с кодом (автодополнение, оптимизация и прочее)]]
    ...
    - [[#горячие-клавиши-1][Горячие клавиши]]
  - [[#aider-как-запускать][Aider. Как запускать]]
    - [[#aider-в-связке-с-ollama-qwen-code][Aider в связке с Ollama (qwen-code)]]
    - [[#aider-в-связке-с-gigachatом][Aider в связке с GigaChat'ом]]
  - [[#agent-shell][Agent-shell]]
  - [[#macher-работа-с-проектом-через-gptel][Macher. Работа с проектом через Gptel]]
- [[#lsp-сервер-vs-eglot][LSP сервер (vs Eglot)]]
  - [[#установка-1][Установка]]
  - [[#всегда-параметр-запуска-отдельным-инстансом][Всегда параметр запуска отдельным инстансом]]
  - [[#общие-настройки][Общие настройки]]
  - [[#клавиши-для-работы][Клавиши для работы]]
  - [[#перезагрузка][Перезагрузка]]
  - [[#copilot][Copilot]]
  - [[#semgrep][Semgrep]]
  - [[#отладка-dap-mode][Отладка (dap-mode)]]
...
- [[#программирование][Программирование]]
  ...
  - [[#показ-кандидатов-через-consult-xref][Показ кандидатов через consult-xref]]
  ...
  - [[#mermaid][Mermaid]]
  - [[#gnuplot][Gnuplot]]
  - [[#typst-более-современный-аналог-latex][Typst. Более современный аналог LaTeX]]
  ...
  - [[#lua][Lua]]
  - [[#java][Java]]
    - [[#поддержка-lombok][Поддержка Lombok]]
    - [[#используем-нативный-lsp-java][Используем нативный lsp-java]]
    - [[#остальные-настройки-под-lsp-java][Остальные настройки под lsp-java]]
    - [[#отладка-dap-java][Отладка (dap-java)]]
    - [[#горячие-клавиши-2][Горячие клавиши]]
    - [[#информация-на-память][Информация на память]]
  - [[#javascripttypescript][JavaScript/TypeScript]]
  - [[#scala][Scala]]
  - [[#perl][Perl]]
- [[#финалочки][Финалочки]]
  - [[#запрещаем-временно-ts-режимы-treesit-поломался][Запрещаем временно ts-режимы (treesit поломался)]]
  - [[#возвращаем-поведение-по-умолчанию-для-переходов-по-ссылкам-в-режиме-редактирования][Возвращаем поведение по умолчанию для переходов по ссылкам в режиме редактирования]]
...
Иногда нужна отладки кода в этом документе в виде печати логов в
специальный буфер.

Создаем буфер =*my/log-buffer*= для записи логов.
...
* Customize в отдельном файле

Customize - это настройки, которые задаются и сохраняются по ходу
дела, а не лежат в этой конфигурации. Например, когда хочется поменять
цветовую тему редактора.

Такие настройки удобнее хранить в отдельном файле, а не добавлять в
[[./init.el]]. Пропишем такой файл как "custom.el".

#+begin_src emacs-lisp :tangle yes
(require 'custom)

(setopt custom-file (expand-file-name (convert-standard-filename "custom.el")
...
Настраиваем редактор на работу в UTF-8. Ниже больше перестраховка,
чтобы уж наверняка.
...
По ходу дела нужно будет сохранять что-то в файлы. Для этого определим
директорию =my/config-dir=. Значение переменной my/config-dir
задается в файле проекта [[./init.el][init.el]].
...
Сторонние пакеты для редактора могут устанавливаться как из локальных
источников, так и с удаленных репозиториев (включая такие, как
GitHub).

Подцепим все такие репозитории.
...
В редакторе по умолчанию показывается меню и панель инструментов. Со
временем они становятся не нужны и их скрывают.

Скроем эти элементы управления и полосы прокрутки текста. Меню
по-прежнему остается доступно по клавише =F10=.
...
Загружаем красивые темы для редактора. Из них можно будет выбирать и
сохранять выбор в Customize.

Для сохранения темы в Customize используем переменную
=my/default-theme= (см. ниже). Аналогично темы по умолчанию прописаны
для демонов (см. ниже в соответствующем разделе).
...
(use-package alect-themes
    :ensure t
    :config
    (setq alect-single-title-height 1.0
...
Устанавливаем одну из тем. Стараемся успеть до загрузки всех остальных
настроек.

#+begin_src emacs-lisp :tangle yes
(defcustom my/default-theme 'doom-one
...
Библиотека, вытянутая из Spacemacs. Так и тащу ее за собой. :)
...
По умолчанию редактор пытается сохранять при выходе расположение окон
и их содержимого. Это больше мешает, чем помогает, поэтому отключим
сохранение среды.
...
Ни разу не был нужен. Отключаем.

#+begin_src emacs-lisp :tangle yes
(setq ring-bell-function 'ignore)
#+end_src

** COMMENT Сборщик мусора (GC)
...
Понижаем до минимального. Спасает от ругани на сертификаты в самых
...
запущенных процессов. Т.к. один из процессов (ws-server - proxy
GigaChat) запущен постоянно, то выключим запрос на подтверждение
остановки процесса, чтобы не вылетало при каждом закрытии редактора.

#+begin_src emacs-lisp :tangle yes
(setq confirm-kill-processes nil)
#+end_src

** Пользовательские переменные окружения (env, exec-path-from-shell)

Подтаскиваем пользовательское окружение из настроек типа ~/.bachrc.
...
Пакет General позволяет удобно задавать клавишу "лидер" (пробел в
режиме EVIL normal или Ctrl-S для родной раскладки Emacs). После
нажатия лидер-клавиши можно вводить остальные цепочки символьных
команд. Например: `SPC f f' - поиск и открытие файла.

Установка клавиш leader (=пробел= или =Alt-m=) и localleader (с
добавкой =m= к лидеру).  Для режима редактирования Emacs лидеры
переопределяются в =Ctrl-s=.
...
** COMMENT Настройка поведения клавиш для evil-mode, как в Vim.
...
** Прокрутка строки к верху окна

После версии 30 курсор стал легко попадать под строку
статуса. Увеличение отступа приводит к тому, что курсор не может
находиться на самых крайних видимых строках, что мешает прокручивать
заголовки вверх окон.

Попробуем сделать костыль на клавиши "z RET" и "z t" для evil-mode.

#+begin_src emacs-lisp :tangle yes
(evil-define-command my/evil-scroll-line-to-top-first-non-blank (count)
    :repeat nil
    :keep-visual t
    (interactive "<c>")
    (let ((scroll-step 1)
...
Назначаем на клавиши.

#+begin_src emacs-lisp :tangle yes
(my/evil-define-key '(normal visual motion) 'override
                    "z <return>" 'my/evil-scroll-line-to-top-first-non-blank
                    "z RET" 'my/evil-scroll-line-to-top-first-non-blank
                    "z t" 'my/evil-scroll-line-to-top-first-non-blank)
#+end_src

* Внешний вид UI и GUI

** Автоматическая установка шрифтов под монитор
...
#+begin_src emacs-lisp :tangle yes
(defcustom my/font-families
  '((:family "Iosevka" :size-dec 9 :size 10 :size-inc 11 :size-hug 14)
    (:family "JetBrains Mono" :size-dec 8 :size 9 :size-inc 10 :size-hug 13)
  ...
  "List of font families with size variants."
  :type '(repeat (list (const :family) string
                  (const :size-dec) integer
                  (const :size) integer
                  (const :size-inc) integer
                  (const :size-hug) integer))
  :group 'my-faces)
#+end_src

Поскольку плотность точек на мониторе не всегда отдается правильно, то
зададим две переменные, позволяющие подкрутить размер шрифтов в нужную
сторону:
- =my/font-size-shift= - прибавит свое значение к размерам шрифтов,
  указанных в структуре my/font-families. Для получения эффекта
  редактор надо будет перезагрузить.
- =my/font-total-scale= - домножает плотность монитора на свое
  значение.

#+begin_src emacs-lisp :tangle yes
(defcustom my/font-size-shift 0
  "Add this value to fonts sizes (integer).
Restart Emacs after changing it."
  :type 'integer)

(defcustom my/font-total-scale 1.0
  "Scale for all fonts (float)."
  :type 'float)
#+end_src
        ...
        (when my/frame-font-families
            (dolist (row my/font-families)
                (let ((family (plist-get row :family))
                      (size-dec (+ (plist-get row :size-dec) my/font-size-shift))
                      (size (+ (plist-get row :size) my/font-size-shift))
                      (size-inc (+ (plist-get row :size-inc) my/font-size-shift))
                      (size-hug (+ (plist-get row :size-hug) my/font-size-shift)))
...
автоматически. Так что предусмотрим "ручное управлене". Помимо
установки шрифта сделаем выравнивание заголовков в org-mode буферах.
...
(defun my/correct-font-in-current-frame ()
    ...
    (my/select-frame-font-for-monitor nil
                                      ...
                                      my/global-font-inc
                                      my/global-font-hug)
    (my/alignment-org-headers-fonts))
...
Установим в статусной панели отображение позиции курсора.
...
Цвет текста в header-line перенастроим, поскольку по-умолчанию он
слишком слепой.
...
каком из них что. Здесь спасает текст в заголовке фрейма. Нужно иметь
возможность назначать текст окна интерактивно. Это будет полезно для
быстрого поиска нужного окна по его заголовку.
...
Нужно в основном во время удаленной демонстраций экрана.
...
В Emacs есть окно компиляции. При запуске какого-либо процесса,
который имеет текстовый выхлоп в стандартный поток вывода, результат
можно тут же видеть в этом окне компиляции. Кроме того, в окне есть
понимание указаний на строки с ошибками и переход к ним в
соответствующих исходниках.
...
- удаление слова по Ctrl+Del.

#+begin_src emacs-lisp :tangle yes
(unbind-key (kbd "C-x C-c"))
(unbind-key (kbd "C-<backspace>"))
(unbind-key (kbd "C-<delete>"))
(unbind-key (kbd "C-DEL"))
(unbind-key (kbd "S-<delete>"))
(unbind-key (kbd "S-DEL"))
#+end_src

Редакция или среда. Здесь задается вызов выравнивания текста по
указанному шаблону.
...
Отдельно сделаем вертикальное деление буфера на две и три части с
follow-mode. Это удобно, когда буфер появляется в нижней части фрейма
и имеет маленький вертикальный размер.

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "w f" '(my/follow-mode-vsplit-2 :which-key "Split 2 follow")
               "w F" '(my/follow-mode-vsplit-3 :which-key "Split 3 follow"))

(defun my/follow-mode-vsplit-2 ()
    (interactive)
    (evil-window-vsplit)
    (follow-mode 1))

(defun my/follow-mode-vsplit-3 ()
    (interactive)
    (evil-window-vsplit)
    (evil-window-vsplit)
    (follow-mode 1))
...
** Vertico — минималистичный интерфейс завершения

=Vertico= — современный минималистичный интерфейс завершения (начинаем
что-то набирать - получаем варианты окончания). Более быстрый и
активно развивается по сравнению с Ivy.

Используется в живом поиске и т.п..
...
(use-package vertico
    ...
    (global-set-key (kbd "C-c C-o") #'embark-collect))
#+end_src

Более удобный способ редактирования путей к файлам в буфере Vertico.
...
# Восстановление результатов последней сессии (отсутствует в Vertico).
...
** Consult — расширенные команды завершения

Меню вызова функции с более полным описанием.

=Consult= предоставляет расширенные команды для работы с завершением.
Заменяет =counsel= и добавляет множество новых функций.
...
Настраиваем предпоказ только для посика по буферу.

#+begin_src emacs-lisp :tangle yes
(consult-customize consult-line :preview-key 'any  ;; '(:debounce 0.3 any)
                   consult-line-multi :preview-key 'any
                   ;; For memory:
                   consult-buffer      :preview-key nil
                   consult-find        :preview-key nil
                   consult-theme       :preview-key nil
                   consult-yank-pop    :preview-key nil)
#+end_src

Горячие клавиши для основных функций Consult.
...
(my/leader-def nil 'override
               ...
               "h t" '(my/consult-theme :which-key "Load visual theme")
...
(defun my/consult-theme ()
    ...
    (unwind-protect
        ...
        (my/correct-fringe-color)))

(defun my/correct-fringe-color ()
...
Строим автодополнения через =Company=.
...
** Marginalia — аннотации кандидатов

=Marginalia= добавляет аннотации (дополнительную информацию) к
кандидатам в меню завершения. Заменяет =ivy-rich=.
...
** Prescient — сортировка и фильтрация по частоте использования

=Prescient= запоминает наиболее часто используемые кандидаты и
сортирует их по частоте использования.
...
Иконки и позиционный фрейм для Vertico.

#+begin_src emacs-lisp :tangle yes
(use-package all-the-icons-completion
    :ensure t
    :config
    (all-the-icons-completion-mode -1))  ;; No icons in minibuffer.
...
** Embark — контекстное меню действий

=Embark= позволяет выполнять действия над выбранными кандидатами.
...
Вернем более привычное перемещение по строкам минибуферов:
Ctrl + jk.
...
** Consult-line. Живой поиск текста

Поиск текста по открытому буферу с результатами в минибуфере.
...
(my/leader-def nil 'override
               ...
               "s S" '(my/consult-line-at-point
                       :which-key "Search buffer for thing at point")
               "s b" '(consult-ripgrep :which-key "Grep or search buffer"))
...
Emacs по-умолчанию не причисляет символ `_' к составу слова, что очень
неудобно. Исправим положение.
...
Создает новую вкладку и интерактивно назначает ей имя. Один прокол -
полоса во вкладками почему-то сразу не показывается. Дерганье
соответствующих функций показа не помогает.
...
Поиск фрейма по имени. Результат - найденный фрейм с указанным именем
или nil. (Для справки: Есть функция =select-frame-by-name=, которая
по имени находит фрейм и активизирует его.)
...
Окна можно делить, закрывать, менять их положение и все в таком духе.
...
Сделаем разделение окон по-заметнее - линия разделения становится
толще.
...
** Запоминаем и восстанавливаем расположение окон

Предусмотрим инструмент сохранения предыдущего состояния окон. Пусть
оно сохраняется в стек =my/window-configuration= и из него же
восстанавливаем. Далее будем использовать этот стек по мере
надобности.

#+begin_src emacs-lisp :tangle yes
(defvar my/window-configuration nil)

(defun my/store-window-configuration ()
    (interactive)
    (push (current-window-configuration) my/window-configuration)
    (message "The windows configuration was stored."))

(defun my/restore-window-configuration ()
    (interactive)
    (when-let (configuration (pop my/window-configuration))
        (message "Previous windows configuration was restored.")
        (set-window-configuration configuration)))
#+end_src

Горячие клавиши на эти две функции.

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "w p p" '(my/store-window-configuration
                         :which-key "Temporary save positions")
               "w p r" '(my/restore-window-configuration
                         :which-key "Restore positions from temprary"))
#+end_src

** Максимизация и восстановление прежнего положения окон

Сделал для случая работы с картинками, а точнее - с рендерингом схем
типа PlantUML. Например, в окне слева редактируется исходный текст
картинки, а в окне справа - строится по тексту сама картинка. И часто
бывает нужно быстро посмотреть эту картинку крупно (если схема
большая) и вернуть окна в прежнее состояние.

Функция ниже сохраняет в стек положение окон и удаляет все окна, кроме
одного.

#+begin_src emacs-lisp :tangle yes
(defun my/maximize-window ()
    (interactive)
    (my/store-window-configuration)
    (delete-other-windows))
#+end_src

Горячие клавиши на максимизацию окна и восстановление обратно.

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "w m t" '(my/maximize-window
                         :which-key "Temporary maximize window")
               "w m T" '(my/restore-window-configuration
                         :which-key "'Minimize' window"))
#+end_src

** Настройка Ediff

Режим ediff позволяет сравнивать содержимое двух или нескольких
буферов, подсвечивая в них соответствующую разницу в тексте.
...
=my/store-window-configuration=, тогда хук =ediff-quit-hook=
восстановит прежнее положение окон.

#+begin_src emacs-lisp :tangle yes
(add-hook 'ediff-quit-hook 'my/restore-window-configuration)
#+end_src
...
(my/leader-def nil 'override
               ...
               "b b" '(consult-projectile-switch-to-buffer
...
(my/leader-def nil 'override
    ...
    "b J" '(consult-buffer-other-window :which-key "Switch buffer with other window"))
#+end_src

** Перенос буферов между окнами

Взято из Spacemacs для переноса буферов между окнами по клавишам
<leader>-b-<Num>.

Нужно, если хочется перенести отображение буфера в другое активное
окно, а в исходном высветить содержимое предыдущего буфера.
...
На широком экране неудобно читать широкие строчки. Сделаем возможность
показа текста в одной центральной суженной колонке.
...
(defun my/scratch-buffer-create (name id)
    (let* ((buffer-name (my/scratch-buffer-name name))
...
#+begin_src emacs-lisp :tangle yes
(defun my/scratch-buffer-name (name)
    (format "*scratch:%s*" name))

(defun my/scratch-buffer-name-perspective ()
    (let* ((name (my/--make-buffer-suffix)))
        (my/scratch-buffer-name name)))
#+end_src
...
(my/leader-def nil 'override
               ...
               "f d" '(find-file :which-key "Open directory or file")
               "f f" '(find-file :which-key "Open file")
               "f r" '(consult-recent-file :which-key "Open recent file")
               "o -" '(dired-jump :which-key "Open directory and select file"))
#+end_src

** Режим ранее открытых файлов

Позволяет выбирать файлы из списка ранее открытых.
...
Позволяет запоминать позицию курсора в ранее открытых файлах.
...
Запретим автоматически обьновлять такие буфера, как Dired (тормозит
редактор).
...
Dired - файловый менеджер, позволяет работать с файлами в
директориях. В сочетании с TRAMP позволяет делать все то же самое на
удаленных машинах.
...
Будем запоминать имена файлов при сохранении и сортировать их по
времени. Результат показываем в org-буфере со ссылками на эти файлы
или их директории.
...
Бывает полезно, если в системе месенджеры оставляют важные сообщения
во всплывающих окнах. Чтобы не искать эти сообщения по месенджерам (не
во всех это удобно), будем логировать их и показывать в отдельном
буфере.

Предполагается, что в системе настроено логирование всплывающих
сообщений типа
...
Функция поиска текста рекурсивно по текущей директории. Под капотом у
нее может быть две альтернативы:
1. =consult-ripgrep= - выводит списками: файл и под ним найденные строки.
2. =counsel-rg= - выводит один список, где имя файла и найденные
   тексты строк находятся в одной строке списка.

#+begin_src emacs-lisp :tangle yes
(defun my/search-in-directory ()
    (interactive)
    (consult-ripgrep default-directory)
    ;; (counsel-rg)  ;; Alternative.
    )
#+end_src
...
Переключение перспектив на соседние.

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "TAB p" '(my/persp-prev :which-key "Previous workspace")
               "TAB n" '(my/persp-next :which-key "Next workspace"))

(defun my/persp-prev ()
    (interactive)
    (persp-prev)
    (my/show-perspective-name))
...
(defun my/make-frame-and-switch-perpective (persp-name)
    (interactive (list (completing-read "Enter workspace name: "
                                        persp-names-cache
                                        nil nil
                                        (my/--get-current-persp-name))))
    (when persp-name
...
(my/leader-def nil 'override
    ...
    "n I" '(my/set-title-bar-and-workspace-interactive :which-key "Title for frame+workspace"))
#+end_src
...
Сам Projectile запоминать ранее открытые проекты не будет. Сделаем это
за него. Определим функцию для запоминания текущего проекта. Будем
вызывать ее по горячим клавишам, не автоматически, чтобы не тормозило
работу с удаленными каталогами.

#+begin_src emacs-lisp :tangle yes
(defun my/projectile-discover-and-save-project (&optional dir)
    (interactive)
    (let ((dir (if dir dir default-directory)))
...
#               (add-hook hook
#                         (lambda ()
#                             (my/projectile-discover-and-save-project default-directory))))
#           hooks))
# #+end_src

Исключаем системные директории для взора projectile.

#+begin_src emacs-lisp :tangle yes
(dolist (dir '("/bin" "/boot" "/dev" "/etc" "/opt" "/proc" "/sys" "/usr" "/var"))
    (add-to-list 'projectile-globally-ignored-directories dir))
#+end_src

И еще одна настройка, которая позволяет при вызове
(projectile-switch-project) открывать директорию проекта, а не
заставлять дальше открывать файл в проекте.

#+begin_src emacs-lisp :tangle yes
(setq projectile-switch-project-action 'projectile-dired)
#+end_src

И еще один костыль. С некоторых пор Projectile стал определять
домашнюю директорию, как проект, что очень неудобно. Отвадим его от
такого.

#+begin_src emacs-lisp :tangle yes
(defun my/projectile-project-root (func &rest args)
    (when-let* ((project-path (apply func args))
                (home-path "~"))
        (if (string= (string-remove-suffix "/" (expand-file-name project-path))
                     (string-remove-suffix "/" (expand-file-name home-path)))
            nil
            project-path)))
...
(my/leader-def nil 'override
    ...
    "p d" '(my/projectile-discover-and-save-project :which-key "Remember project")
    "p p" '(my/projectile-switch-project :which-key "Simple switch project")
    ...
    "p f" '(consult-projectile-find-file :which-key "Open file in project")
    ...
    "p n" '(projectile-index-project-async :which-key "Indexate")
...
(setq grep-find-ignored-directories
      '("SCCS" "RCS" "CVS" "MCVS" ".src" ".svn" ".git" ".hg" ".bzr"
        "_MTN" "_darcs" "{arch}"
        "__pycache__" "build" "develop-eggs"
        "dist" "dist_local" "eggs" ".eggs" "sdist" ".pytest_cache"
        "env" ".env" "venv" ".venv" "ENV" ".ENV"
        "env.bak" ".env.bak" "venv.bak" ".venv.bak"
        ".mypy_cache" "build" ".ccls-cache"))
(setq grep-find-ignored-files
      (append grep-find-ignored-files '("TAGS" "tags")))
(setq helm-grep-ignored-directories
      '("SCCS/" "RCS/" "CVS/" "MCVS/" ".svn/" ".git/" ".hg/" ".bzr/"
        ...
        "dist/" "dist_local/" "eggs/" ".eggs/" "sdist/" ".pytest_cache/"
        "env/" ".env/" "venv/" ".venv/" "ENV/" ".ENV/"
        "env.bak/" ".env.back/" "venv.bak/" ".venv.bak/"
        ".mypy_cache/" "build/" ".ccls-cache"))
(setq projectile-globally-ignored-directories
      '("~/.emacs.d/.local/" "~/.emacs.d/.local/" "~/.emacs.d/.local/"
...
Emacs может быть запущен в режиме демона (не на всех ОС). В этом
случае графически можно подключаться к демону разными фреймами
клиентов. Клиенты по-прежнему работают в одном потоке демона, но в
разных фреймах. Можно завязывать отдельного клиента на отдельное
рабочее пространство, что позволяет работать одновременно с разными
проектами. После запуска демона клиенты запускаются мгновенно.
...
- демон "notes" (записи)     - "теплая" тема,
- демон "dev"   (разработка) - "холодная" тема,
- демон "rest"  (данные)     - тема с "прозрачностью".
Так сложнее визуально перепутать клиентские фреймы для каждого из
демонов.

Разбиение работы по разным демонам позволяет им не мешать друг другу.

Заведем переменные подобные my/default-theme.

#+begin_src emacs-lisp :tangle yes
(defcustom my/default-theme-notes 'doom-old-hope
  "Default visual theme for daemon `notes'."
  :type '(symbol))

(defcustom my/default-theme-dev 'doom-one
  "Default visual theme for daemon `dev'."
  :type '(symbol))

(defcustom my/default-theme-rest 'doom-one
  "Default visual theme for daemon `rest'."
  :type '(symbol))
#+end_src
...
(pcase (daemonp)
    ("notes"
     (load-theme my/default-theme-notes t))

    ("dev"
     (load-theme my/default-theme-dev t))

    ("rest"
     (setq my/transparency-level 1)
     (load-theme my/default-theme-rest t)))
#+end_src
...
При старте демонов "Notes" (записи) или "rest" (данные, исследования,
запросы), мне удобно сразу переименовать первый клиентский фрейм и
создать одноименное рабочее пространство. Автоматизируем эти действия,
немного подстраховавшись на случай запуска [[id:cc0595b8-0628-4bd8-b398-ddfc80b95a01][клиентских сценариев]].
...
Org - мощный пакет для работы с текстом в org-формате. Формат похож на
markdown, но если markdown предполагает чтение в сверстанном виде
(рендеринг), то org заточен под работу в сыром виде (plain text).

Редактор предоставляет формату org мощную поддержку, которую можно
расширять самому. Таблицы и списки, ссылки, агенда, формулы, ведение
дел, экспорт в разные форматы и многое другое.

Отдельно стоить подсветить работу с кусочками кода прямо в тексте org:
их выполнение, объединение в цепочки, поддержка разных языков,
поддержка сессионности и прочее. Это напоминает Jupyter-notebook, но с
одновременной работой на разных языках программирования.

Также в org удобно работать в парадигме литературного программирования
(LP). Пример LP - данный конфиг редактора.
...
Начиная с какого-то обновления, =flycheck= теперь гоняет =org-lint= на
каждый введенный символ, чем сильно тормозит редактор и сыпет ошибками
(возможны даже зависания). Отключим flycheck для режима org-mode.
...
** Сквозные идентификаторы в org (ID и CUSTOM_ID)

Каждому заголовку в org (а также самому org-файлу) можно назначить
идентификатор (в формате uuid). Эти сквозные идентификаторы можно
использовать в перекрестных ссылках. Например на этом строится работа
пакета Org-Roam.

Такие идентификаторы проставляет функция org-id-copy, но она делает
только идентификатор с именем ID.
...
Вызов календарика из любого буфера.
...
Не всегда функция org-ctrl-c-ret работает так, как хочется. Например,
если на родительских уровнях списка есть разделительные пустые строки,
то и новый пункт списка будет сделан с добавлением перед ним пустой
строки, которую приходится удалять. Это неудобно.

Сделаем функцию, которая создает новый пункт списка, не добавляя
пустую строку. Если функция вызвана на пустой строке, то новый элемент
списка будет создан на этой же строке. И параметр для чекбокса.

#+begin_src emacs-lisp :tangle yes
(defun my/org-new-list-item-new-line-check ()
    (interactive)
    (my/org-new-list-item-new-line t))

(defun my/org-new-list-item-new-line (&optional check)
    (interactive)
    (unless (string-empty-p (my/get-current-line-trim))
        (evil-open-below 0))
    (my/org-new-list-item check))

(defun my/org-new-list-item (&optional check)
    (interactive)
...
(defun my/get-current-line-trim ()
    (string-trim
     (buffer-substring-no-properties (line-beginning-position)
                                     (line-end-position))))
#+end_src

Повесим создание нового элемента списка на быстрые клавиши.

#+begin_src emacs-lisp :tangle yes
(define-key org-mode-map (kbd "M-o") 'my/org-new-list-item-new-line)
(define-key org-mode-map (kbd "M-O") 'my/org-new-list-item-new-line-check)
#+end_src
...
** Разнообразная реакция на Enter (RET, dwim)

При нажатии Enter попробуем узнать тип объекта под курсором и в
зависимости от него вызвать соответствующую функцию.

Сперва отключим переход на RET по ссылкам при редактировании. С
некоторых пор какой-то из пакетов устанавливает переменную
=org-return-follows-link= в t. В [[*Финалочки]["Финалочках"]] отключим это.
...
** Разнообразная реакция на двойное нажатие мышки (mouse, dwim)
...
** Открытие org-ссылки в том же окне и в окне соседнем
...
#+begin_src emacs-lisp :tangle yes
(when-let* ((pair (assq 'file org-link-frame-setup)))
    (setcdr pair 'find-file))
#+end_src

Однако, оставим возможность переходить по ссылке в другом окне.

#+begin_src emacs-lisp :tangle yes
(defun my/link-to-other-window ()
    (interactive)
    (when-let* ((type (my/object-type-at-point))
                (is-link (member type '(link link-image))))
        (save-excursion
            (switch-to-buffer-other-window (current-buffer)))
        (if (eq type 'link-image)
            (my/open-picture-in-same-window)
            (my/dwim-at-point))))

(define-key org-mode-map (kbd "S-<return>") nil)
(define-key org-mode-map (kbd "S-RET") nil)

(my/evil-define-key '(normal visual motion) 'org-mode-map
                    "S-<return>" 'my/link-to-other-window)
(my/evil-define-key '(normal visual motion) 'org-mode-map
                    "S-RET" 'my/link-to-other-window)
#+end_src
...
Org-babel блок - это блок текста, заключенный в операторные скобки
`#+begin_src' и `#+end_src' с параметрами. Обычно в них помещается
выполняемый код на каком-либо языке программирования (Python и пр.)
или около того (PlantUML, GNU Plot и пр.). Результат выполнения такого
кода появляется тут же в секции `RESULT:' под блоком.

Выполнение блоков можно объединять в цепочки, где результат одного
блока является входными данными для другого. Обычно внутренне org
пытается представить (по возможности) результат, как список или список
списков (таблица). Такие структуры есть в каждом ЯП.

Также есть поддержка сессионности, как в Jupyter-Notebook.
...
Начало рабочей недели с понедельника:
...
#+begin_src emacs-lisp :tangle yes
(defun my/individual-visibility-source-blocks (&optional start end)
...
С 30-й версии Emacs схлопывание выше перестало работать на
org-mode-hook. Повесим это счастье на =org-cycle-hook=.

#+begin_src emacs-lisp :tangle yes
(add-hook 'org-cycle-hook 'my/fold-blocks-on-org-cycle)

(defun my/fold-blocks-on-org-cycle (state)
    (if-let* ((local-p (not (eq 'all state)))
              (element (org-element-at-point))
              (begin (org-element-property :contents-begin element))
              (end (org-element-property :contents-end element)))
        (my/individual-visibility-source-blocks begin end)
...
Org-файлы можно экспортировать в разные форматы (ODТ, LaTeX, PDF и пр.).
...
Если в org нужно нарисовать формулы, то можно воспользоваться
инструментами LaTeX. Редактор будет рендерить эти формулы в картинку
прямо в тексте.
...
Размер картинки с формулой в org-mode сильно зависит от физического
разрешения монитора. Из-за этого она может оказаться слишком маленькой
или большой рядом с остальным текстом. Поправить это можно лишь
вручную, подобрав параметр =:scale= в свойствах
=org-format-latex-options=. Проще и лучше всего это сделать через
customize.

Ниже сделаем костыль, если customize трогать не хочется.
...
Быстрое вытаскивание ссылки из org-текста. Ссылка копируется сразу в
три регистра: `+', `*' и `"'.
...
(defun my/open-picture-in-same-window ()
    ...
    (when-let* ((element (org-element-context))
                ...
                (raw-link (org-element-property :raw-link element))
                (file (my/extract-file-name-from-link raw-link)))
        (find-file file)))

(defun my/extract-file-name-from-link (file-link)
    (if (string= "file" (car (split-string file-link ":")))
        (substring file-link (length "file:"))
        file-link))
#+end_src
...
Для показа интерактивных презентаций прямо в org воспользуемся пакетом
=org-tree-slide=.
...
Очень часто открываю новый буфер, после чего устанавливаюему режим
org-mode. Автоматизируем эти действия.
...
Agenda - это список дел. Список делается на основании org-заголовков,
начинающихся с "TODO" и прочих состояний задачи. Если в свойствах
заголовка задачи указано время выполнения, то задача может попасть в
агенду. Самое простое - это указать файлы с задачами агедны в
переменной =org-agenda-files= (Customize в помощь).
...
Здесь определяются нужные мне статусы задач (TODO, DONE и прочие).
...
** Org-transclusion. Показ цитат из других источников без их копирования
...
Для памяти (если долго не буду использовать):
- Перед ссылкой в начале строки ставим текст =#+transclude:=. Таким
  он в тексте и останется.
- Чтобы показать цитату, нужно поставить курсор на строку с этой
  ссылкой и вызвать функцию =org-transcludion-add=. Появиться цитата и
  будет она в read-only.
- Можно показать сразу все цитаты в буфере функцией
  =org-transcludion-add-all=.
- Убираем показ цитат функциями =org-transclusion-remove= и
  =org-transclusion-remove-all=.
- Обновить цитаты можно функцией =org-transclusion-refresh=.
...
По-умолчанию цитата ничем не отличается от остального текста в
org. Ниже выделяем цитату цветом на полях.

#+begin_src emacs-lisp :tangle yes
(set-face-attribute 'org-transclusion-fringe
                    nil
                    :foreground "white"
                    :background "darkgray"
                    :weight 'bold
...
#+begin_src emacs-lisp :tangle yes
(my/localleader-def nil 'org-mode-map
                    "c" '(:ignore t :which-key "Transclusion")
                    "c a" '(org-transclusion-add :which-key "Add")
                    "c A" '(org-transclusion-add-all :which-key "Add all")
                    "c m" '(org-transclusion-mode :which-key "Mode")
                    "c S" '(org-transclusion-demote-subtree
                            :which-key "Demote subtree")
                    "c O" '(org-transclusion-move-to-source
                            :which-key "Move to source")
                    "c P" '(org-transclusion-promote-subtree
                            :which-key "Promote subtree")
                    "c d" '(org-transclusion-remove :which-key "Reomve")
                    "c D" '(org-transclusion-remove-all :which-key "Reomve all")
                    "c C-d" '(org-transclusion-detach :which-key "Detach")
                    "c e" '(org-transclusion-live-sync-start :which-key "Sync start")
                    "c g" '(org-transclusion-refresh :which-key "Refresh")
                    "c o" '(org-transclusion-open-source :which-key "Open source"))
#+end_src

** Унификация шрифтов в заголовках

В части тем нет способа регулировать размер шрифтов в заголовках. Я же
предпочитаю везде одинаковый шрифт. Сделаем принудительную унификацию
шрифтов в заголовках.
...
(defun my/alignment-org-headers-fonts ()
    (when (boundp 'org-level-faces)
...
Org-Roam - это пакет для работы с заметками в формате org,
поддерживающий работу над перекрестными ссылками между ними. Заметкой
(узлом) может быть как файл, так и любой org-заголовок в нем. Для
этого достаточно назначить заголовку сквозной идентификатор (функция
org-id-copy).

Пакет похож на Obsidian, конечно, со своими особенностями. Как и все в
Emacs, функционал пакета можно расширять самостоятельно.

На основе Org-Roam легко сделать личную википедию или картотеку. Такое
очень полезно на работе. Для удобства сделал поддержку нескольких баз
заметок (vault), не пересекающихся друг с другом.

Из функционала Org-Roam:
- показ обратных ссылок,
- тегирование узлов (с наследованием тегов от родительских узлов),
- текстовый поиск по заголовкам и тегам,
- поддержка daily-заметок (быстро записать, а позже определить на
  место).

Для ссылочного графа есть свой GUI, но практического смысла в нем
мало.
...
Некоторые теги неудобно наследовать от родительских узлов к
дочерним. Например тег документов-списков "<list": если его ноды
находятся в его же файле, то все они этот тег унаследуют.
...
Шаблоны нужны для создания новых документов. Шаблон удобно задать
текстовым файлом и связать его с соответствующей директорией (дневник,
исследования, люди и т.п.). Работу с такими файлами-шаблонами
поддержим ниже.
...
Помимо основного тега "@main" добавим не наследуемый тег "<base".
...
Помимо основного тега "@daily" добавим не наследуемый тег "<base".
...
Помимо основного тега директории добавим не наследуемый тег "<base".
    ...
    (let* ((subdirectory-name (string-replace " " "-"
        ...
        `(,key
          ...
          (file+head ,(concat subdirectory-name "/%<%Y%m%d%H%M%S>-${slug}.org")
                     ,(concat "#+title: ${title}" "\n"
                              "#+date: %<%Y-%m-%d>" "\n"
                              "#+filetags: :@" subdirectory-name ":<base:" "\n"))
          :unnarrowed t
          :empty-lines-before 1)))
...
По умолчанию Org-Roam работает с директорией, указанной в переменной
=org-roam-directory=. Это означает работу только с одной заметочной
базой. А было бы удобно иметь несколько не пересекающихся баз. Сделать
это можно на основе переменных директории, определяемых в файле
.dir-locals.el. Сделаем поддержку раздельных заметочных баз, как это
можно делать в Obsidian через vault.
...
Красиво, но не практично, поскольку проект не доработан в мелочах.
...
Позволяет делать запросы к заметкам Org-Roam и показывать результат
как в отдельном окне, так и в таблице прямо в org-документе. Ниже
информация о том, как это делать в данной конфигурации.
...
Здесь собраны терминалы, встроенные в Emacs. Поддержка их в данном
разделе заключается не только в вызове самого терминала, но и
сопоставлении терминала проекту или рабочему пространству по
следующему правилу:
1. Если в текущей директории обнаружен проект, то терминал создается
   под этот проект.
2. Если проект не найден, но определено рабочее пространство, то
   терминал создается под это пространство.
3. Во всех остальных случаях получаем общий терминал.

** eshell

Eshell - это не просто терминал, но и одновременно интерпретатор
emacs-lisp. Одна из его полезностей заключается в том, что он не
создает сессий, и при обрыве соединения с удаленной машиной попросту
налаживает связь заново при вводе команды. Это позволяет держать
постоянно открытый терминал eshell к уделенной машине. Связь с
удаленными машинами держит в связке с TRAMP.
...
каждого проекта или рабочего пространства. Если буфер с таким
терминалом уже создан, то он показывается в окне или окно с таким
терминалом скрывается.

#+begin_src emacs-lisp :tangle yes
(defun my/popup-eshell-toggle ()
    (interactive)
    (my/eshell-toggle 'pop-to-buffer))

(defun my/eshell-replace-toggle ()
    (interactive)
    (my/eshell-toggle 'switch-to-buffer))

(defun my/eshell-toggle (switch-func)
    (let* ((buffer-name (format "*eshell-popup:%s*"
        ...
        (if-let ((window (get-buffer-window buffer)))
            ...
            (funcall switch-func buffer))))

(defun my/scratch-buffer-name-eshell ()
...
(my/leader-def nil 'override
    ...
    "o E" #'my/eshell-replace-toggle)
#+end_src

** shell

В отличие от eshell создает сессию. При работе на удаленных машинах
это дает возможность не мешать удаленное окружение с локальным.

Вызов терминала shell. Терминал создается по возможности свой для
каждого проекта или рабочего пространства. Если буфер с таким
терминалом уже создан, то он показывается в окне.

#+begin_src emacs-lisp :tangle yes
(defun my/popup-shell-toggle ()
    (interactive)
    (my/shell-toggle 'pop-to-buffer))

(defun my/shell-replace-toggle ()
    (interactive)
    (my/shell-toggle 'switch-to-buffer))

(defun my/shell-toggle (switch-func)
    (let* ((buffer-name (my/scratch-buffer-name-shell))
           (buffer (get-buffer buffer-name)))
        (if buffer
            (funcall switch-func buffer)
...
(defun my/scratch-buffer-name-shell ()
...
(my/leader-def nil 'override
    ...
    "o S" #'my/shell-replace-toggle)
#+end_src

** vterm

Не эмитирует терминал, а использует настоящий, но в окошке Emacs. Это
дает возможность не зависеть от особенностей Emacs. В остальном все то
же самое.
...
Дружим Vterm c Evil. Для этого надо отключить vi-режим в терминале.

#+begin_src emacs-lisp :tangle yes
(setopt vterm-keymap-exceptions
        '("C-c" "C-x" "C-u" "C-g" "C-h" "C-l"
          "M-x" "M-o" "C-v" "M-v" "C-y" "M-y" "C-w"
          "C-\\" "C-r"))

(defun my/vterm--set-emacs-mode ()
    (vterm-send-string " set -o emacs\n"))

(add-hook 'vterm-mode-hook 'my/vterm--set-emacs-mode)
#+end_src
...
(defun my/popup-vterm-toggle ()
    (interactive)
    (my/vterm-toggle 'pop-to-buffer))

(defun my/vterm-replace-toggle ()
    (interactive)
    (my/vterm-toggle 'switch-to-buffer))

(defun my/vterm-toggle (switch-func)
    (let* ((buffer-name (my/scratch-buffer-name-vterm))
           (buffer (get-buffer buffer-name)))
        (if buffer
            (funcall switch-func buffer)
            (let ((buffer (get-buffer-create buffer-name)))
...
(defun my/scratch-buffer-name-vterm ()
    (format "*vterm-popup:%s*"
            (my/--make-buffer-suffix)))
#+end_src

Запилим горячие клавиши на вызов окошка vterm.

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
    "o t" #'my/popup-vterm-toggle
    "o T" #'my/vterm-replace-toggle)
#+end_src

** eat

Рассматриваю Eat, как альтернативу Vterm. Однако, оказывается менее
удобен в EVIL.
...
Здесь просто запомню хуки для интеграции с Eat. Пока она мне не
нужна.
...
Вызов терминала Eat. Терминал создается по возможности свой для
каждого проекта или рабочего пространства. Если буфер с таким
терминалом уже создан, то он показывается в окне или окно с таким
терминалом скрывается.

#+begin_src emacs-lisp :tangle yes
(defun my/popup-eat-toggle ()
    (interactive)
    (my/eat-toggle 'pop-to-buffer))

(defun my/eat-replace-toggle ()
    (interactive)
    (my/eat-toggle 'switch-to-buffer))

(defun my/eat-toggle (switch-func)
    (let* ((eat-buffer-name (format "*eat-popup:%s*"
                                    (my/--make-buffer-suffix)))
           (buffer (get-buffer eat-buffer-name))
           (window))
        (unless buffer
            (setq buffer (get-buffer-create eat-buffer-name))
            (with-current-buffer buffer
                (eat)))
        (if-let ((window (get-buffer-window buffer)))
            (delete-window window))
...
(defun my/scratch-buffer-name-eat ()
    (format "*eat-popup:%s*"
            (my/--make-buffer-suffix)))
#+end_src

Запилим горячие клавиши на вызов окошка Eat.

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
    "o l" #'my/popup-eat-toggle
    "o L" #'my/eat-replace-toggle)
#+end_src

** Асинхронный вызов системных команд (async-shell-command)

Рисует текстовый выхлоп в буфере, как Compile, но асинхронно.
...
TRAMP дает возможность использовать почти все инструменты Emacs (такие
как терминалы, проводники, редактирование файлов, запуск процессов и
многое другое) на удаленных машинах, в докерах, а также в докерах на
удаленных машинах.

Это дает возможность, например, оставить открытый буфер Dired, консоль
Eshell или Docker-интерфейс на удаленной машине/докере и не
беспокоиться об обрывах связи (например, при отправке компьютера в
сон). В сочетании с рабочими пространствами получаем "панель
управления" удаленной машиной.

Проще всего воспользоваться TRAMP'ом - это открыть удаленную
директорию в Dired (или просто открыть на редакцию файл на удаленной
машине). Далее в удаленном контексте работают все инструменты Emacs.

Примеры использования TRAMP (набираем в строке ввода):
- Докер на удаленной машине:
  : /ssh:my-user@my-host|docker:root@tmy-container:/my-path/
- Работа под sudo:
  : /sudo::/etc/profile
...
** Ielm. REPL для emacs-lisp

Повесим на горячие клавиши.

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "o i" '(ielm :which-key "Ielm REPL"))
#+end_src

** Eww. Встроенный в Emacs браузер

Браузер, встроенный в Emacs, без поддержки JS. Имеет свою полезность.

Если поставить курсор на URL, то запуск eww сразу подставит его в
строку запроса браузера перед открытием. Этим удобно, например,
смотреть картинки по ссылкам. Это достойно горячей клавиши.

Горячие клавиши.

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "o b" '(eww :which-key "EWW Browser"))
#+end_src

** Treemacs

Дерево проектов, как принято в IDE. Полезная штука для анализа
незнакомого кода.
...
Уберем перенос строк.

#+begin_src emacs-lisp :tangle yes
(add-hook 'treemacs-mode-hook 'my/--unwrap-words)
#+end_src
...
(define-key treemacs-mode-map (kbd "C-c C-p u") 'treemacs-refresh)
#+end_src
...
Интерфейс для работы с докерами (start, stop, inspect, консоль и
пр.). В сочетании с TRAMP позволяет управлять докерами на удаленной
машине.
...
/В пакете были изменения и клавиши под них пока не настроены./

Включение модуля "calendar" позволяет смотреть свои дела из Agenda в
виде обычного календаря.

#+begin_src emacs-lisp :tangle yes
(use-package calfw
	:ensure t
	:config
    (setq calfw-render-line-breaker 'calfw-render-line-breaker-simple
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
    "o a c" #'calfw-org-open-calendar)
#+end_src

Вызов Agenda для выбранного дня:

#+begin_src emacs-lisp :tangle yes
(define-key calfw-calendar-mode-map "d" #'calfw-org-open-agenda-day)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(defun my/calfw-open-org-calendar-current-file ()
    (interactive)
    (let ((org-agenda-files (list (buffer-file-name))))
        (calfw-org-open-calendar)))

(my/leader-def nil 'override
               "o a C" '(my/calfw-open-org-calendar-current-file :which-key "Calendar for current file"))
#+end_src
...
Пакет дает примеры запуска различных программ и утилит. Спасает от
долгих исследований man-страниц.
...
Настраиваем пакет web-server для создания своих web-серверов внутри
Emacs. Это пригодится ниже для написания proxy-сервера прд GigaChat.
...
Получаем у операционной системы свободный порт.

#+begin_src emacs-lisp :tangle yes
(defun my/get-free-port ()
    (when-let ((process (make-network-process :name "temp-free-port"
                                              :server t
                                              :host "127.0.0.1"
                                              :service t)))
        (let* ((result (process-contact process :local))
               (port (aref result (1- (length result)))))
            (delete-process process)
            port)))
#+end_src

Чтобы сделать из ws-server proxy-сервер, мне не хватает некоторых
ws-методов. Сделаем их.

Подготовим вспомогательные функции для работы с aslit.

#+begin_src emacs-lisp :tangle yes
(defun my/member-assoc (key alist)
    "Like member but for alists. Return tail."
    (my/member-assoc-predicate (lambda (x)
                                   (eq x key))
                               alist))

(defun my/member-assoc-predicate (key-predicate alist)
    (when alist
        (if (funcall key-predicate (caar alist))
            alist
            (my/member-assoc-predicate key-predicate (cdr alist)))))

(defun my/take-before-key (key alist)
    "Take elements from alist before element (key . x)."
    (my/take-before (lambda (x-key)
                        (eq x-key key))
                    alist))

(defun my/take-before (key-predicate alist)
    (when alist
        (let ((x (car alist))
              (xs (cdr alist)))
            (if (funcall key-predicate (car x))
                nil
                (cons x (my/take-before key-predicate xs))))))

(defun my/remove (key alist)
    (when alist
        (cl-remove-if (lambda (pair)
                          (equal key (car pair)))
                      alist)))

(defun my/insert-or-update (key val alist)
    (when alist
        (cons (cons key val)
              (my/remove key alist))))
#+end_src

Первая партия для перхода с ws-server на url-retrieve.

#+begin_src emacs-lisp :tangle yes
(defun my/ws-method-str (request)
    "Return method like \"GET\", \"POST\", ..."
    (when-let* ((method (my/ws-method request)))
        (string-remove-prefix ":" (symbol-name method))))

(defun my/ws-method (request)
    "Return method like :GET, :POST, ..."
    (when-let* ((headers (ws-headers request))
                (method (cadr headers)))
        (car method)))

(defun my/ws-uri (request)
    "Return URI line '/api/v1'."
    (when-let* ((headers (ws-headers request))
                (method (cadr headers)))
        (cdr method)))

(defun my/ws-url-params-str (request)
    (when-let* ((prarms (my/ws-url-params request)))
        (string-join (mapcar (lambda (pair)
                                 (concat (car pair)
                                         "="
                                         (url-encode-url (cdr pair))))
                             (my/ws-url-params request))
                     "&")))

(defun my/ws-url-params (request)
    (when-let* ((headers (ws-headers request)))
        (my/take-before (lambda (x)
                            (not (stringp x)))
                        (cddr headers))))

(defun my/ws-headers-only-strs (request)
    "Return headers like ((\"Content-Type\" . \"text/application\") ...)."
    (let ((headers-only (my/ws-headers-only request)))
        (mapcar (lambda (header)
                    (cons (my/key-to-header (car header))
                          (cdr header)))
                headers-only)))

(defun my/ws-headers-only (request)
    "Return headers like ((:CONTENT-TYPE . \"text/application\") ...)."
    (when-let* ((headers (ws-headers request))
                (host-member (my/member-assoc-predicate 'symbolp
                                                        (cddr headers)))
                (key-p (lambda (key)
                           (not (keywordp key)))))
        (my/take-before key-p
                        (my/remove :HOST host-member))))

(defun my/key-to-header (header-key)
    "Convert keyword like :CONTENT-TYPE to string like \"Content-Type\"."
    (when (keywordp header-key)
        (string-join
         (mapcar 'capitalize
                 (string-split (string-remove-prefix ":"
                                                     (symbol-name header-key))
                               "-"))
         "-")))
#+end_src

Вторая партия для перехода с url-retrieve на ws-server.

#+begin_src emacs-lisp :tangle yes
(defun my/url-code ()
    (save-excursion
        (goto-char (point-min))
        (when-let* ((line (buffer-substring (point) (line-end-position)))
                    (parts (string-split line))
                    (success (> (length parts) 1))
                    (code (string-to-number (cadr parts)))
                    (success (> code 0)))
            code)))

(defun my/url-headers ()
    (save-excursion
        (goto-char (point-min))
        (forward-line 1)
        (when-let* ((headers-begin (point))
                    (headers-end (my/url-goto-body))
                    (headers-text (buffer-substring-no-properties headers-begin
                                                                  headers-end))
                    (headers-strings (string-split
                                      (string-replace "\r\n" "\n" headers-text)
                                      "\n"))
                    (headers (mapcar 'my/header-line-to-pair headers-strings)))
            (cl-remove-if (lambda (pair)
                              (or (string-empty-p (car pair))
...
(defun my/url-goto-body ()
    (goto-char (if-let ((pos (search-forward "\n\n" nil t)))
                   pos
                   (point-max))))

(defun my/header-line-to-pair (line)
    (let* ((parts (string-split line ":"))
           (header-key (capitalize (string-trim (car parts))))
           (header-val (string-trim (string-join (cdr parts) ":"))))
        (cons header-key header-val)))
#+end_src
...
StartDict - это утилита для работы со словарями. Редактор интегрирует ее.
...
Пространство со словариком и окошком черновика для слов.

#+begin_src emacs-lisp :tangle yes
(defun my/switch-to-scratch-perspective ()
    (interactive)
    (if-let* ((buffer-name-perspective (my/scratch-buffer-name-perspective))
              (buffer (get-buffer buffer-name-perspective))
              (window (get-buffer-window buffer)))
        (select-window window)
        (my/make-scratch-perspective))
...
(my/leader-def nil 'sdcv-mode-map
               "SPC" '(my/switch-to-scratch-perspective
                       :which-key "Open scratch buffer"))
#+end_src

** Калькулятор на Python

Здесь создается живой Python-калькулятор на основе пакета
=live-py-mode=. Принцип простой: в окне слева набираем выражения на
Python, а в окне справа сразу видим результат выражений (как состояние
переменных). Удобен не только, как калькулятор, но и как быстрая
проверка работы кода на Python.

Кроме того пакет =live-py-mode= позволяет писать python-тесты и сразу
видеть их результат во время написания.
...
** Читалка в консоли (RAG)
   :PROPERTIES:
   :ID:       a95627b0-aa7b-40db-80a9-daf7c8975114
   :CUSTOM_ID: a95627b0-aa7b-40db-80a9-daf7c8975114
   :END:

#+begin_src emacs-lisp :tangle yes
(defcustom my/reader-ask-command
  "head"
  "Command for run reader like: reader file."
  :type '(string))

(defun my/reader-find-book ()
    (interactive)
    (my/reader--open-shell (my/reader--find-file))
    (my/markdown-minor-mode))

(defun my/reader--find-file ()
    (expand-file-name
     (read-file-name "Select index file (*.db): "
                     nil nil t nil
                     (lambda (name) (or (file-directory-p name)
                                        (string-match-p "\\.db\\'" name))))))

(defun my/reader--open-shell (index-file-name)
    (when-let* ((buffer-name (format "BOOK-COMPANY: %s"
                                     (file-name-base index-file-name)))
                (buffer (pop-to-buffer buffer-name)))
        (eshell-mode)
        (goto-char (point-max))
        (insert (concat my/reader-ask-command
                        " "
                        (eshell-quote-argument index-file-name)))
        (eshell-queue-input)
        (eshell-send-input)
        ))

(my/leader-def nil 'override
               "o r" '(my/reader-find-book :which-key "Open book reader"))
#+end_src
...
Вызывается клиент с окном живого Python-калькулятора.
...
Панель на старте с логотипом нужна не только для красоты, но как и
способ возврата из текущего контекста TRAMP. Например, буфер
принадлежит контексту на удаленной машине. Чтобы быстро переключиться
на локальный контекст, просто вызываем dashboard-буфер.

Панель содержит картинку и немного подсказок. Мне этого достаточно -
все остальное не прижилось.
...
(setq dashboard-banner-logo-title
      (concat "\n\n"
              ...
              "\n\n\n"
              (format "Open file                                  %s\n"
                      (my/get-function-key 'my/find-file))
              (format "Recent file                                %s\n"
                      (my/get-function-key 'consult-recent-file))
              "\n"
...
Часто бывает нужно попасть на нейтральный буфер, например когда нужно
переключиться из сессии в TRAMP. Для этого повесим вызов буфера с
Dashboard на горячие клавиши.

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "o h" '(my/switch-to-home :which-key "Dashboard"))
...
Здесь будем подцеплять нужные инструменты для работы с ИИ (точнее с
LLM). Один из таких инструментов - пакет Gptel. Он позволяет выбирать
разные ИИ модели в качестве бекэнда, сохраняя единый интерфейс.

** Чтение ключей из gpg-файлов

Для подключения к API облачных моделей обычно нужны ключи.
...
Приложение Ollama позволяет работать с локально установленными
моделями. Здесь отдельный пакет для разговоров через Ollama.
...
(defun my/gptel-refresh-backend (backend-name)
    (interactive (list (completing-read "Enter backend name: "
...
** GPTel. Выбор модели заданного бекэнда

 Функция выбора модели у указанного бекэнда. При вызове сперва
 обновляет список моделей.

#+begin_src emacs-lisp :tangle yes
(defun my/gptel--select-backend-model (backend-name)
    (when-let* ((make-backend-func (gethash backend-name my/gptel--backends-hash))
                (backend (funcall make-backend-func backend-name))
                (models-symbols (gptel-backend-models backend))
                (models-names (mapcar 'symbol-name models-symbols))
                (model (completing-read "Select model: "
                                        models-names
                                        nil nil
                                        (car models-names)))
                (model (my/find-symbol-with-same-name (intern model)
                                                      models-symbols)))
        (setq gptel-backend backend)
...
Здесь основное - это определить, какие модели в Ollama сейчас
загружены. Делаем это автоматом с возможностью повторного ручного
чтения списка моделей (функция my/gptel-refresh-backend).
...
Повесим быстрое переключение на Ollama на горячие клавиши. При
переключении сначала обновится список локальных моделей, а потом будет
предложено выбрать одну из них.

Пригодится, если есть возможность запускать локальные модели.

#+begin_src emacs-lisp :tangle yes
(defun my/gptel-switch-to-ollama ()
    (interactive)
    (my/gptel--select-backend-model "Ollama"))

(my/leader-def nil 'override
               "l g L" '(my/gptel-switch-to-ollama :which-key "Switch to Ollama"))
#+end_src

** GPTel. Подключаем GigaChat

У GigaChat есть своя особенность - одного ключа недостаточно, нужно
еще по нему получить токен и уже с ним ходить в API. Это затрудняет
его работу со стандартными инструментами. Ниже написан костыль для
подключения к Gptel, а также proxy-сервер, позволяющий использовать
API GigaChat для разных инструментов типа ИИ-агентов работы с кодом.
...
(defun my/gptel--session-id ()
    (unless (boundp 'chat-session-id)
        (setq-local chat-session-id my/giga-session-id)
...
пользоваться curl'ом только для буферов с +GigaChat+ - для всех
буферов, с Ollama тоже фокусы.
...
** Proxy в API Gigachat

Будем хранить URL к Гиге и фальшивый ключ в этих переменных.

#+begin_src emacs-lisp :tangle yes
(defvar my/giga--url-base nil
  "URL base for GigaChat API proxy.")

(defvar my/giga--proxy-key nil
  "False key for GigaChat API proxy.")

(defvar my/giga-model-best "GigaChat-2-Max"
  "GigaChat best model.")

(defvar my/giga-session-id "f2b72500-f1ee-42c6-b743-a16766bc130f"
  "Value for X-Session-ID header.")
#+end_src

Функция запуска GigaChat proxy-сервера.

#+begin_src emacs-lisp :tangle yes
(defun my/giga-start-proxy-server-env (&optional force)
    "Start proxy server on a random port.
Save URL base and the server key in variables:
    `my/giga--url-base'
    `my/giga--proxy-key'
The function save the params to environment variables:
    `GIGACHAT_URL_BASE'
    `GIGACHAT_PROXY_KEY'"
    (interactive)
    (when (or force
              (not (and my/giga--url-base
                        my/giga--proxy-key)))
        (my/giga-start-proxy-server))
    (setenv "GIGACHAT_URL_BASE" my/giga--url-base)
    (setenv "GIGACHAT_PROXY_KEY" my/giga--proxy-key))

(defun my/giga-start-proxy-server ()
    "Start proxy server on a random port.
Save URL base and the server key in variables:
    `my/giga--url-base'
    `my/giga--proxy-key'"
    (when-let ((port (my/giga--start-proxy-server))
               (key (base64-encode-string (encode-coding-string (org-id-uuid)
                                                                'utf-8))))
        (setq my/giga--url-base (format "http://localhost:%d" port)
              my/giga--proxy-key key)
        (message (format "GigaChat API proxy server start: %s"
                         my/giga--url-base))))
#+end_src

Web-server не умеет справляться с заголовком авторизации с "Bearer":
: Authorization: Bearer <my_key>
Сделаем костыль на функцию ws-parse.

#+begin_src emacs-lisp :tangle yes
(advice-add #'ws-parse :around 'my/giga--remove-bearer)

(defun my/giga--remove-bearer (func proc str &rest args)
    (let ((new-str (if (stringp str)
                       (string-replace "Authorization: Bearer "
                                       "Authorization: "
                                       str)
                       str)))
        (apply func proc new-str args)))
#+end_src

Еще один костыль. Исправляет ошибку в ws-parse-request. Здесь случай,
когда запрос от aider приходит несколькими чанками.

#+begin_src emacs-lisp :tangle yes
(advice-add #'ws-parse-request :around 'my/ws-parse-request-fixed)

(defun my/ws-parse-request-fixed (func request)
  "Wrapper around ws-parse-request that checks if complete body is received."
  (when (funcall func request)
      (let* ((headers (my/ws-headers-only-strs request))
             (content-length (alist-get "Content-Length" headers nil nil 'string=))
             (content-length (if (stringp content-length) (string-to-number content-length) 0))
             (body (ws-body request))
             (body-length (string-bytes body))
...
Строим proxy-сервер. Функция =my/giga--start-proxy-server= будет
запускать его на случайном порту.

#+begin_src emacs-lisp :tangle yes
(defun my/giga--start-proxy-server ()
    "Start proxy server on a random port. Return the port."
    (let* ((port (my/get-free-port)))
        (ws-start 'my/giga--proxy-handler
                  port)
        port))

(defun my/giga--proxy-handler (request)
    (if-let* ((process (ws-process request))
              (headers-in (my/ws-headers-only-strs request))
              (proxy-key (my/giga--extract-proxy-key headers-in))
              (is-authorized (equal proxy-key my/giga--proxy-key)))
        (let* ((url-request-data (my/giga--request-data request))
               (url-request-method (my/ws-method-str request))
               (url-request-extra-headers (my/giga--insert-session-id-to
                                           (my/giga--insert-key-to-headers
                                            (my/giga--correct-contet-length
                                             url-request-data
                                             headers-in))))
               (url (my/giga--make-url request))
               (response-buffer (url-retrieve-synchronously
                                 url)))
            (with-current-buffer response-buffer
                (let* ((headers-out (my/remove "Transfer-Encoding"
                                               (my/url-headers))))
                    (apply 'ws-response-header
                           process
                           (my/url-code)
                           headers-out))
                (process-send-string process (my/url-body))
                (kill-buffer response-buffer)))
        (progn (message "Unauthorized")
               (ws-response-header process 401))))

(defun my/giga--extract-proxy-key (headers-alist)
    (when-let* ((key-header (assoc "Authorization" headers-alist 'equal))
                (key-part (string-trim (cdr key-header))))
        (string-remove-prefix "Bearer " key-part)))

(defun my/giga--make-url (request)
    (string-join (cl-remove-if 'not
                               (list (concat "https://gigachat.devices.sberbank.ru/api/v1"
                                             (my/ws-uri request))
                                     (my/ws-url-params-str request)))
                 "?"))

(defun my/giga--insert-session-id-to (headers-alist)
    (my/insert-or-update "X-Session-ID"
                         my/giga-session-id
                         headers-alist))

(defun my/giga--insert-key-to-headers (headers-alist)
    (my/insert-or-update "Authorization"
                         (concat "Bearer " (my/gptel-gigachat-access-token))
                         headers-alist))

(defun my/giga--correct-contet-length (body headers-alist)
    (my/remove "Content-Length"
               headers-alist))

(defun my/giga--request-data (request)
    (let* ((request-json (ws-body request)))
        (my/giga--correct-request-data request-json)))

(defun my/giga--correct-request-data (request-json)
    (let* ((request-json (if (or (not request-json)
                                 (string-empty-p request-json))
                             (my/giga--make-empty-request)
                             request-json))
           (data (json-read-from-string request-json))
           (data (my/giga--correct-model-name data))
           ;; (data (my/giga--correct-stream-type data))
           (data (my/giga--correct-system-pormpt-place data)))
        (json-encode data)))

(defun my/giga--make-empty-request ()
    (concat "{\"model\":\"" my/giga-model-best "\""
            ",\"messages\":[{\"role\":\"user\"}]"
            ",\"stream\":true"
            ",\"temperature\":0"
            "}"))

(defun my/giga--correct-model-name (data)
    (let* ((model-name (alist-get 'model data))
           (model-name (if (and model-name
                                (string-search "GigaChat" model-name))
                           model-name
                           my/giga-model-best))
           (data-remove-model (my/remove 'model data))
           (data-add-gigachat-model (cons `(model . ,model-name)
                                         data-remove-model)))
        data-add-gigachat-model))

(defun my/giga--correct-stream-type (data)
    (let* ((data-remove-stream-mode (my/remove 'stream data))
           (data-add-stream-mode (cons '(stream . :json-false)
                                       data-remove-stream-mode)))
        data-add-stream-mode))

(defun my/giga--correct-system-pormpt-place (data)
    (if-let* ((system-p (lambda (el) (and (string= "system"
                                                   (alist-get 'role el)))))
              (messages (alist-get 'messages data))
              (systems (seq-filter system-p messages))
              (last-system (car (last systems)))
              (messages (seq-remove system-p messages))
              (messages (cons last-system messages))
              (messages (apply 'vector messages)))
        (my/insert-or-update 'messages messages data)
        data))

(defun my/giga--debug-request-to-string (method url headers data)
    (let ((method-str (if (symbolp method) (symbol-name method) method)))
        (concat method-str " " url
                "\n" (my/giga--debug-headers-to-string headers)
                "\n\n" data
                "\n")))

(defun my/giga--debug-headers-to-string (headers)
    (string-join (mapcar (lambda (pair)
                             (format "%s: %s"
                                     (car pair)
                                     (cdr pair)))
...
Сервер будем *стартовать* при запуске редактора.

#+begin_src emacs-lisp :tangle yes
(my/giga-start-proxy-server-env)
#+end_src

** Работа с кодом (автодополнение, оптимизация и прочее)

Сделаем "плагин" для бесед с LLM'ками через Gptel. В появляющемся окне
справа будут беседы: один первоуровневый заголовок - одна беседа. Все
внутри одной беседы попадает в контекстное окно. Вопросы - заголовки
второго уровня или весь последний параграф. Ответы - содержимое
параграфов.
...
(defun my/llm-codework-open-out-buffer ()
    (interactive)
    (my/llm-codework-switch-to-out-buffer my/llm-codework-out-buffer))
    ...
    (let* ((created-p (get-buffer out-buffer))
        ...
        (with-current-buffer buffer
            (unless created-p
                ...
                (org-mode)
                (when (file-exists-p my/llm-codework-out-buffer-file)
                    (insert-file-contents my/llm-codework-out-buffer-file)
                    (org-cycle-global))))))
#+end_src
...
возвращает такой диалог для дальнейшей передачи в =gptel-request=.
    ...
    (let ((lang (car (string-split (symbol-name major-mode) "-"))))
        (cond
          ((string= lang "emacs")
           "emacs-lisp")
          ((string= lang "org")
           (my/get-src-block-lang))
          ((string= lang "cperl")
           "perl")
          (t
           lang))))

(defun my/get-src-block-lang ()
...
#+begin_src emacs-lisp :tangle yes
(defun my/llm-codework-complete (lang)
    (interactive (list (read-string "Language: "
                                    (my/get-lang))))
    (let* ((prompt "")
           (system (format "Complete or finish the following code written in %s that starts after the <src_code_block> tag and ends before the </src_code_block> tag. Do not modify or add comments to the original code. Use only objects and variables present in the source code; do not introduce new ones. If you need more context to complete the task, request additional preceding code. Include the result along with the original code. Replace the tags <src_code_block> and </src_code_block> with #+begin_src %s and #+end_src, respectively. Provide your answer in Russian."
                           lang lang)))
           ;; (system (format "Дополни или заверши приведенный ниже код на %s, который начинается после тега <src_code_block> и завершается перед тегом </src_code_block>. Исходный код не исправляй и не дополняй комментариями. В ответе используй только те объекты и переменные, которые есть в исходном коде, других не придумывай. Если для выполнения задачи тебе не хватает контекста, то ответом запроси нужный тебе предшествующий код. Результат приведи вместе с исходным кодом. Теги <src_code_block> и </src_code_block> в своем ответе замени на операторные скобки #+begin_src %s и #+end_src. Ответ дай по русски."
           ;;                 lang lang)))
        (my/llm-codework-base (format "(%s) Completion of Code" lang)
                              system
                              prompt)))

(defun my/llm-codework-completion-variants (lang)
    (interactive (list (read-string "Language: "
                                    (my/get-lang))))
    (let* ((prompt "")
           (system (format "Complete or finish the following code written in %s that starts after the <src_code_block> tag and ends before the </src_code_block> tag. Do not modify or add comments to the original code. Use only objects and variables present in the given code; do not introduce new ones. Provide your result along with the original code. Create 1 to 5 versions of the answer and number them accordingly. Include the original code in your response. Replace the tags <src_code_block> and </src_code_block> with #+begin_src %s and #+end_src, respectively. Provide your answer in Russian."
                           lang lang)))
        ;; (system (format "Дополни или заверши приведенный ниже код на %s, который начинается после тега <src_code_block> и завершается перед тегом </src_code_block>. Исходный код не исправляй и не дополняй комментариями. В ответе используй только те объекты и переменные, которые есть в исходном коде, других не придумывай. Результат приведи вместе с исходным кодом. Сделай таким образом от 1 до 5 вариантов ответа и пронумеруй их. Результат приведи вместе с исходным кодом. Теги <src_code_block> и </src_code_block> в своем ответе замени на операторные скобки #+begin_src %s и #+end_src. Ответ дай по русски."
        ;;                 lang)))
        (my/llm-codework-base (format "(%s) Completion Variants of Code" lang)
                              system
                              prompt)))

(defun my/llm-codework-optimize (lang)
    (interactive (list (read-string "Language: "
                                    (my/get-lang))))
    (let* ((prompt "")
           (system (format "Optimize the following code written in %s, which starts after <src_code_block> tag and ends before </src_code_block> tag. Below provide an explanation of your optimization. If no optimization is needed, indicate this in your response without changing the original code. You can use more efficient solutions or libraries. Replace tags <src_code_block> and </src_code_block> with block delimiters #+begin_src %s and #+end_src. Provide your answer in Russian."
                           lang lang)))
        ;; (system (format "Оптимизируй код на %s, приведенный ниже, который начинается после тега <src_code_block> и завершается перед тегом </src_code_block>. Ниже приведи объяснение своей оптимизации. Если оптимизации не требуется, сообщи об этом в ответе, а не меняй исходный код. Можешь использовать более эффективные решения или библиотеки.  Теги <src_code_block> и </src_code_block> в своем ответе замени на операторные скобки #+begin_src %s и #+end_src. Ответ дай по русски."
        ;;                 lang)))
        (my/llm-codework-base (format "(%s) Optimization of Code" lang)
                              system
                              prompt)))

(defun my/llm-codework-explain (lang)
    (interactive (list (read-string "Language: "
                                    (my/get-lang))))
    (let* ((prompt "")
...
(defun my/llm-codework-inquire-further ()
    ...
    (when (and (string= (buffer-name) my/llm-codework-out-buffer)
        ...
        (let* ((lang (my/get-lang))
            ...
            (my/llm-codework--insert-wait-text)
            (gptel-request
                prompts
                :stream nil
                :system system
...
#+begin_src emacs-lisp :tangle yes
(defun my/llm-codework-remember (lang)
    (interactive (list (read-string "Language: "
                                    (my/get-lang))))
    (when (use-region-p)
...
И самый простое - начать новую тему без вопроса.

#+begin_src emacs-lisp :tangle yes
(defun my/llm-codework-new-theme (lang)
    (interactive (list (read-string "Language: "
                                    (my/get-lang))))
    (my/llm-codework-switch-to-out-buffer my/llm-codework-out-buffer)
    (goto-char (point-max))
...
(defun my/llm-codework-callback (header text out-buffer response _info)
    (when (and response
        ...
        (my/llm-codework--delete-wait (plist-get _info :position))
        (insert (gptel--convert-markdown->org response))
        (insert "\n")))

(setq my/llm-codework--wait-text "Wait ...")

(defun my/llm-codework--insert-wait-text ()
    (save-excursion
        (insert my/llm-codework--wait-text)))

(defun my/llm-codework--delete-wait (wait-position)
    (when-let* ((wait-length (length my/llm-codework--wait-text))
                (begin (if wait-position wait-position (point)))
                (begin (save-excursion (goto-char begin)
                                       (min begin (line-beginning-position))))
                (end (min (point-max) (+ begin wait-length)))
                (wait-text (buffer-substring-no-properties begin end))
                (wait-text-p (string= wait-text my/llm-codework--wait-text)))
        (goto-char begin)
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "i" '(:ignore t :which-key "Code AI work")
               "i i" '(my/llm-codework-open-out-buffer
                       :which-key "Open out buffer")
               "i I" '(my/llm-codework-toggle-out-buffer
                       :which-key "Toggle out buffer")
...
Aider - ИИ-агент для работы с кодом. Здесь его поддержка для Emacs.

*** Aider в связке с Ollama (qwen-code)

    Будем пользоваться моделью =qwen2.5-coder=.
...
OLLAMA_API_BASE=http://127.0.0.1:11434 aider \
                                       ...
                                       $@
    #+end_src
    Для удобства это можно положить в скрипт запуска:
    - [[~/.local/bin/aider-ollama-qwencode.sh]]

*** Aider в связке с GigaChat'ом

- Подготовка ::

  1. Положить настройки для GigaChat в файл:
     : ~/.aider.model.metadata.json
     Настройки следующие (показаны только для одной модели):
     #+begin_src json :eval no
{
    "openai/GigaChat-2-Max": {
        ...
        "input_cost_per_token": 0.000023,
        "output_cost_per_token": 0.000023,
...
- Как пользоваться ::
  ...
  1. Запускаем proxy-сервер:
     : M-x my/giga-start-proxy-server-env
     После запуска URL и ключ появятся в переменных среды редактора.

  2. Открываем консоль в Emacs:
     : M-x vterm
     Можно воспользоваться и shell с eshell, но будет некрасиво, а
     главное - не будет работать режим --watch-files.

  3. Запускаем из консоли aider:
     #+begin_src shell :eval no
aider --model openai/GigaChat-2-Max \
      --model-metadata-file ~/.aider.model.metadata.json \
      --openai-api-base $GIGACHAT_URL_BASE \
      --openai-api-key $GIGACHAT_PROXY_KEY \
      --watch-files \
      --dark-mode \
      $@
     #+end_src
     Для удобства это можно положить в скрипт запуска:
     - [[~/.local/bin/aider-gigachat-emacs.sh]]

** Agent-shell

Среда для работы с кучей инструментов типа Curson, Claude-code,
Opencode и других. Страница на GitHub:
- https://github.com/xenodium/agent-shell?tab=readme-ov-file

Требуется установить нужный инструмент на машину, далее подцепится к
нему автоматически.

*Памятка*: Для переключения агентов внутри инструментов используется
клавиша =Ctrl+TAB=.
...
Настроим перевод строки в evil-mode, как это показано в инструкции.
...
Настроим, чтобы окно agent-shell появлялось в соседнем окне.
По-умолчанию буфер агента появляется в текущем окне:
: (display-buffer-same-window)

#+begin_src emacs-lisp :tangle yes
(setopt agent-shell-display-action '(display-buffer-in-previous-window))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "l a" '(:ignore t :which-key "agent-shell")
               "l a a" '(agent-shell :which-key "Select agent")
               "l a d" '(agent-shell-anthropic-start-claude-code
                         :which-key "Claude-Code")
               "l a o" '(agent-shell-opencode-start-agent
                         :which-key "OpenCode")
               "l a p" '(agent-shell-pi-start-agent
                         :which-key "Pi")
               "l a u" '(agent-shell-cursor-start-agent
                         :which-key "Cursor"))

(define-key agent-shell-mode-map (kbd "M-RET") 'agent-shell-ui-toggle-fragment)
#+end_src

** Macher. Работа с проектом через Gptel

Macher - это агент для работы с много-файловыми проектами, подобно
Aider, Pi или Claude, но нативный для Emacs. В качестве бекэнда для
моделей Macher использует Gptel. Контекстное окно Macher держит в
org-буфере, дополняя его по мере необходимости. Заголовки в этом
буфере - вопросы и задания. При необходимости что-то изменить в
проекте Macher создает diff-буфер, который можно применить командой
diff-apply-buffer (с записью в файлах) или diff-apply-hunk (только в
буфере).

Ниже привожу немного сценариев использования Macher:

- Планирование выполнения задачи ::
  1) Вызываем macher-discuss и вводим постановку задачи в
     минибуфере. Если текст большой и вводить его в минибуфере
     неудобно, то можно приготовить его где угодно, выделить и вызвать
     macher-discuss.
  2) В отдельном буфере Macher повторит запрос в виде org-заголовка,
     выведет нужные контекст и даст ответ. Если ответ
     неудовлетворителен, то можно продолжить диалог с моделью через
     masher-discuss или прямо в окне, вводя запросы в новых
     подзаголовках.
  3) Удовлетворительный ответ может понадобиться сохранить в файл или
     дать команду на выполнение. Для этого используем команду
     macher-implement ровно таким же образом, как и macher-discuss.
     Вместо явного вызова macher-discuss можно в начале запроса
     поставить пресет @macher. Пример:
     : @macher Хорошо. Теперь выполни задание.
  4) Если diff чем-то не удовлетворяет, то можно сделать уточнение
     через macher-revise.
  5) Если с diff все хорошо, применяем к нему diff-apply-buffer или
     diff-apply-hunk.

- Выполнить задачу ::
  + То же самое, что и в предыдущем сценарии, но начинаем сразу с
    macher-implement.

- Поправить ошибку в выхлопе или упавший тест ::
  1) Выделяем выхлоп, содержащий информацию о проблеме, и вызываем
     macher-implement.
  2) Далее, как везде.
...
(use-package macher
    ...
    (macher-enable)
    :hook
    (macher-action-buffer-setup . my/macher--on-buffer-setup-hook)
...
(defun my/macher--on-buffer-setup-hook ()
    (setq-local window-point-insertion-type t)
    (visual-line-mode 1))
#+end_src

Костыль для применения diff-apply-buffer. В Emacs 30 эта функция не
умеет работать с файлами (создавать и переименовывать). Но можно
воспользоваться внешней командой "git apply -" (разумеется, только для
проектов Git).

#+begin_src emacs-lisp :tangle yes
(defun my/git-apply-buffer ()
    (interactive)
    (if (= emacs-major-version 30)
        (let ((default-directory (my/macher-get-default-directory)))
            (if (zerop (my/get-git-apply-region-status))
                (message "Patch applied successfully.")
                (message "Patch failed. See *git-apply-errors* for details.")))
        (diff-apply-buffer)))

(defun my/macher-get-default-directory ()
    (or (when-let ((proj (project-current)))
            (project-root proj))
        (vc-root-dir)
        default-directory))

(defun my/get-git-apply-region-status ()
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "l m" '(:ignore t :which-key "macher")
               "l m k" '(macher-abort :which-key "Abort request")
               "l m a" '(macher-action :whick-key "Action")
               "l m d" '(macher-discuss :which-key "Discuss")
               "l m f" '(macher-focus-string :which-key "Focus string")
               "l m i" '(macher-implement :which-key "Implement")
               "l m q" '(macher-process-request :which-key "Process request")
               "l m w" '(macher-process-request-dwim :whick-key "DWIM process request")
               "l m r" '(macher-revise :which-key "Revise")
               "l m y" '(my/git-apply-buffer :which-key "Apply diff buffer"))
#+end_src
...
Расширение и сужение колонок в tablist-mode.

#+begin_src emacs-lisp :tangle yes
(my/evil-define-key nil 'flycheck-error-list-mode-map
                    ">" 'tablist-enlarge-column
                    "<" 'tablist-shrink-column)
#+end_src

** Всегда параметр запуска отдельным инстансом

Сделаем передачу параметра для отдельного инстанса сервера постоянным.

#+begin_src emacs-lisp :tangle yes
(advice-add #'lsp :around
            (lambda (func &optional is-new)
                (funcall func t)))
#+end_src
...
(defun my/eldoc ()
    (interactive)
    (let* ((lsp-mode-p (member 'lsp-mode minor-mode-list))
           (doc-buffer-name (if lsp-mode-p
                                "*lsp-help*"
                                "*eldoc*"))
           (current-buffer-name (buffer-name (current-buffer)))
           (doc-func (if lsp-mode-p
                         'lsp-describe-thing-at-point
                         (lambda () (eldoc t))))
           (doc-window (get-buffer-window doc-buffer-name)))
        (if (or doc-window
                (member current-buffer-name '("*lsp-help*" "*eldoc*")))
            (delete-window doc-window)
            (funcall doc-func))))

(my/evil-define-key '(normal motion) 'lsp-mode-map  ;; 'prog-mode-map
                    "RET" #'lsp-ui-doc-show)
(my/evil-define-key '(normal motion) 'override
                    "K" #'my/eldoc
                    "g K" #'eldoc)
#+end_src

Показ текста eldoc в минибуфере иногда может сильно нагрузить
редактор. Поэтому нужна функция отключения.

#+begin_src emacs-lisp :tangle yes
(defun my/eldoc-toggle-hover ()
    (interactive)
    (setq lsp-eldoc-enable-hover (not lsp-eldoc-enable-hover)))
#+end_src
...
И для ситуаций, когда вмешивается магия.
...
Lsp-mode подсвечивает объекты, на которых находится курсор. Добавим
клавиш для перемещения по ним.

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'lsp-mode-map
               "c p" '(lsp-ui-find-prev-reference :which-key "Find previous reference")
               "c n" '(lsp-ui-find-next-reference :which-key "Find next reference"))
#+end_src
...
(my/leader-def nil 'override
               ...
               "c l d" '(lsp-describe-session :which-key "Describe session")
               "c l e" '(lsp-treemacs-errors-list :which-key "Errors list")
               "c l a" '(lsp-execute-code-action :which-key "Execute code action")
               "c l m" '(lsp-workspace-folders-remove :which-key "Remove workspace")
               "c l b" '(lsp-workspace-blocklist-remove :which-key "Blocklist remove")
               "c l w" '(lsp-workspace-restart :which-key "Restart workspace")
               "c l h" '(my/eldoc-toggle-hover :which-key "Toggle eldoc hover"))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "c l l" '(my/lsp :which-key "Run LSP server")
               "c l L" '(my/lsp-with-reset-cache
                         :which-key "Run LSP server, clear cache")
               "c l s" '(my/lsp-shutdown-workspace :which-key "Shutdown LSP workspace")
               "c l r" '(my/lsp-restart :which-key "Restart LSP"))
#+end_src
...
Отладчик. Давно перестал этим пользоваться, но вдруг пригодится.
...
(use-package dap-mode
    ...
    (setq dap-auto-configure-features '(sessions
                                        locals
                                        breakpoints
                                        expressions
                                        controls
...
Удобные управляющие клавиши для отладки есть в dap-hydra. Повесим ее
вызов на горячие клавиши.
...
Tree sitter - это инструмент для создания парсеров. Он позволяет
построить синтаксическое дерево по тексту (коду). Редактор, опираясь
на tree sitter, может отказаться от использования регулярных выражений
при редактировании и тем самым ускорить свою работу и получить
дополнительный функционал.

Для каждого языка программирования на основе tree sitter может быть
написан свой парсер. Такие парсеры в этой конфигурации накатываются
отдельно под каждый язык. В этих случаях обычно наряду с привычными
функциями режимов типа `python-mode' появляются новые, типа
`python-ts-mode', и далее пользоваться лучше ими (конечно, если режим
под tree-sitter хорошо написан).
...
(defun my/treesit-install-language-grammar (lang-symbol github-path)
    (let ((element (if (listp github-path)
...
Горячие клавиши. Не все -ts-режимы хороши или просто присутствуют в
пакетах. Поэтому можно вызывать =treesit-explore-mode= для получения
минорного treesit-режима.

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "c t" '(:ignore t :which-key "treesit")
               "c t e" '(treesit-explore-mode :which-key "Explore mode")
               "c t i" '(treesit-inspect-mode :which-key "Inspect mode"))
#+end_src
...
Здесь собраны инструменты для работы с кодом (редактирование,
компиляция и пр.). Также здесь работаем с разными форматами текста,
поскольку грань между текстом и кодом бывает расплывчатой (пример -
PlantUML). Особняком тут держится работа с PDF.

** Навигация по ошибкам

Ошибки в коде выделяются редактором по общему для всех
механизму. Запилим горячие клавиши для перемещения по ошибкам на
основе flycheck (а для C++/C когда-то - flymake, но это устарело и
присутствует в виде рудимента).

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "c e" '(:ignore t :which-key "errors")
               "c e p" '(my/jump-previous-error :which-key "Jump to previous error")
               "c e n" '(my/jump-next-error :which-key "Jump to next error")
               "c e a" '(flycheck-list-errors :which-key "List errors (flycheck)"))
...
Для компиляции чего либо можно вызвать функцию =compile=. Она
позволить ввести команду для выполнения, а выхлоп команды будет
собирать в буфер compile. Там же в буфере можно переходить на ошибки в
исходниках.
...
** Показ кандидатов через consult-xref

Embark-consult интегрирует xref с Consult.
...
Минорный режим для подсветки markdown в других режимах. Пригождается в
консольных режимах, когда много печати в стилистике markdown (например
в eshell).

#+begin_src emacs-lisp :tangle yes
(define-minor-mode my/markdown-minor-mode
    "Markdown highligthing in current buffer."
    :lighter " MD"
    (if my/markdown-minor-mode
        (progn
            (font-lock-add-keywords nil markdown-mode-font-lock-keywords)
            (font-lock-flush)
            (toggle-word-wrap 1))
        (font-lock-remove-keywords nil markdown-mode-font-lock-keywords)
        (font-lock-flush)
        (toggle-word-wrap nil)))
#+end_src

Горячие клавиши, чтобы вызывать набрасывать markdown-подсветку на
текущий буфер.

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "v" '(:ignore t :which-key "view and highlight")
               "v m" '(my/markdown-minor-mode :which-key "Markdown minor mode"))
#+end_src
...
Работа с Json: подсветка, навигация, разбор, форматирование и прочее.
...
Позаботимся о просмотре json в отформатированном виде. Для этого
запилим форматирующую функцию.

#+begin_src emacs-lisp :tangle yes
(defun my/json-pretty-format (body)
    (with-temp-buffer
        (insert body)
        (let ((json-encoding-pretty-print t)
              (json-encoding-default-indentation "    "))  ;; 4 spaces
            (json-pretty-print-buffer)
            (buffer-string))))
#+end_src

Вычисление org-babel json-блоков, результат которых будет
отформатированный json.

#+begin_src emacs-lisp :tangle yes
(defun org-babel-execute:json (body params)
  "Format JSON string in BODY according to PARAMS.
If BODY is a valid JSON string, it is pretty-printed with 4-space indentation.
If BODY is a Elisp object, it is converted to JSON and then pretty-printed."
  (condition-case nil
      (my/json-pretty-format body)
      (error (error "Invalid JSON in source block"))))
#+end_src

Сделаем еще одну вещь - отформатированный вывод выделенного json'а в
отдельном окне.

#+begin_src emacs-lisp :tangle yes
(defun my/json-region-pretty-show ()
    (interactive)
    (if (use-region-p)
        (let* ((json-pretty-buffer-name "*json-pretty-show*")
               (begin (use-region-beginning))
               (end (use-region-end))
               (body (buffer-substring-no-properties begin end))
               (json-pretty (my/json-pretty-format body))
               (buffer-created-p (get-buffer json-pretty-buffer-name))
               (buffer (get-buffer-create json-pretty-buffer-name)))
            (with-current-buffer buffer
                (erase-buffer)
                (unless buffer-created-p
                    (json-ts-mode))
                (insert json-pretty))
            (switch-to-buffer-other-window buffer))
        (message "Please, select regin with json.")))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "c f j" '(my/json-region-pretty-show :which-key "Json pretty show"))
#+end_src

** Make

Подсветка make-файлов.
...
Наряду с plantuml-mode можно рисовать диаграммы в org-файлах,
используя src-блоки - сформированная картинка с диаграммой будет
результатом выполнения такого блока.
...
** Typst. Более современный аналог LaTeX

Ссылки и пакеты:
- О самом typst: [[https://github.com/typst/typst][Typst]]
  + Документация: [[https://typst.app/docs/][Typst. Documentation]]
- Пакет: [[https://codeberg.org/meow_king/typst-ts-mode][typst-ts-mode]]
- Org preview: [[https://github.com/remimimimimi/org-typst-preview.el][org-typst-preview]]
- LSP server: [[https://myriad-dreamin.github.io/tinymist/frontend/emacs.html][Tinymist. Install for Emacs]]
  + Альтернатива от Anaconda: [[https://anaconda.org/conda-forge/typst-lsp][typst-lsp]]
...
Установим основные пакеты. К сожалению, режим =py-shell-mode= после
обновлений вышел из-под контроля и мешается везде. Из-за этого
исключим =python-mode= из установки - его заменит =python-ts-mode= из
treesit.
...
Pyright лучше установить в системе через npm (не полагаясь на lsp-install-server).
1. Установить сам npm:
   : sudo pacman -S npm
2. Установить Pyright
...
Устанавливаем поддержку Pyright.
...
/На данный момент появился uv-mode. Пока пробуем использовать его./
...
(defun my/pyvenv-select-path ()
    ...
    (if (= 0 (hash-table-count my/pyvenv-activated-paths))
        ...
        (let ((path (completing-read "Select Python virtual environment path:"
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "c v" '(:ignore t :which-key "python venv")
               "c v a" '(my/pyvenv-activate :which-key "Activate venv")
               "c v i" '(my/pyvenv-select-path :which-key "Select venv")
               "c v d" '(pyvenv-deactivate :which-key "Deactivate venv"))
#+end_src

** COMMENT Python. UV-mode

Выглядит, как замена pyvenv. Установим.
...
#+begin_src emacs-lisp :tangle yes
(unless (boundp 'haskell-process-args-ghci)
    (setopt haskell-process-args-ghci nil))
...
;; (add-hook 'haskell-mode-hook #'my/treesit-minor-mode)
;; (add-hook 'haskell-literate-mode-hook #'my/treesit-minor-mode)

;; (defun my/treesit-minor-mode ()
;;     ;;; Empty here...
...
Настройка LSP-сервера. Достаточно просто установить =gopls= в систему:
: sudo pacman -S gopls
...
Устанавливаем LSP-сервер.

Официальный LSP-сервер для Kotlin находится тут:
- https://github.com/Kotlin/kotlin-lsp/tree/main
Проект проще собрать в системе. Например в Arch через AUR пакет:
: yay -S kotlin-lsp-bin
Сервер будет доступен по команде
: kotlin-lsp
Это на самом деле ссылка на bash-скрипт по адресу:
: /usr/share/kotlin/kotlin-lsp/kotlin-lsp.sh

Помощь в настройке Emacs находится в том же репозитории:
- https://github.com/Kotlin/kotlin-lsp/blob/main/scripts/lsp-kotlin-emacs-eglot.el
По ней написана конфигурация ниже.
...
Местный kotlin-ls, работающий по-умолчанию с пакетом lsp-mode,
мешается и может вызываться невпопад. Исключим его.

#+begin_src emacs-lisp :tangle yes
(delete 'kotlin-ls lsp-client-packages)
#+end_src

Как для Java разделяем технические файлы проектов по собственным
директориям. Параметры для облегчения работы lsp-ui стоит
раскомментировать, если понадобится.

#+begin_src emacs-lisp :tangle yes
(defun my/run-kotlin-lsp ()
    (setq-local ;; lsp-modeline-diagnostics-enable nil  ;; Too hard for jdtls.
                ;; lsp-modeline-code-actions-enable nil
                ;; lsp-modeline-workspace-status-enable nil
                ;; lsp-java-progress-reports-enabled nil
                ;; lsp-ui-sideline-enable nil
                lsp-kotlin-workspace-dir (my/java-lsp--get-data-dir)  ;; Unique data.
                )
    (lsp t))  ;; Different instance for each project.

(add-hook 'kotlin-mode-hook 'my/run-kotlin-lsp)
#+end_src
...
Сперва позаботимся о поддержке lombok. Копируем lombok.jar в кеш
редактора.
...
*** Используем нативный lsp-java

/Остановился на этом варианте. JDTLS нужно прогрузить вручную/
/(lsp-install-server), а в логах встречаются выбросы исключений. Но/
/зато работает все и есть переходы на код определений из сторонних/
/библиотек./

Делаем установку. В переменной =lsp-java-jdt-download-url=
*ОБЯЗАТЕЛЬНО!* надо указать последнюю версию JDTLS.
...
(use-package lsp-java
    ...
    (setq lsp-java-jdt-download-url
          "https://www.eclipse.org/downloads/download.php?file=/jdtls/milestones/1.56.0/jdt-language-server-1.56.0-202601291528.tar.gz")
    :config
    (when (member 'lsp-java-boot features)
        (require 'lsp-java-boot))
    ;; For Lombok:
    (add-to-list 'lsp-java-vmargs (concat "-javaagent:" my/java-lombok))
    (add-to-list 'lsp-java-vmargs "-Dlombok.disableConfig=true")
    (add-to-list 'lsp-java-vmargs "-Dlombok.anyConstructor.suppressConstructorProperties=false")
    (add-to-list 'lsp-java-vmargs "-Dlombok.addLombokGeneratedAnnotation=true")
    (add-to-list 'lsp-java-vmargs "-Dlombok.extern.findbugs.addSuppressFBWarnings=false")
    ;; ;; Increase resources for Spring Boot:
    (setq lsp-java-vmargs (remove "-Xmx1G" lsp-java-vmargs))
    (add-to-list 'lsp-java-vmargs "-Xmx4G")
    (add-to-list 'lsp-java-vmargs "-XX:+UseStringDeduplication")
    ;; (add-to-list 'lsp-java-vmargs "-XX:+UseG1GC")
    ;; (add-to-list 'lsp-java-vmargs "-Xms1G")
    ;; (add-to-list 'lsp-java-vmargs "-XX:GCTimeRatio=4")
    )
#+end_src

*** COMMENT Регистрируем новый LSP-сервер (python launch)

/Здесь полностью рабочий вариант. Чего не смог от него добиться - это/
/перехода на код определений из сторонних библиотек./

/JDTLS запускаем через приложенный python скрипт./

Где качать (или собирать) версию JDTLS можно прочитать на их [[https://github.com/eclipse-jdtls/eclipse.jdt.ls][странице]]
на GitHub. Я скачал готовую сборку вот отсюда: [[https://download.eclipse.org/jdtls/snapshots/?d][Eclipse Downloads]] и
сохранил архив в данный репозиторий. Его, соответственно, надо будет
обновлять по необходимости.

Сохраняем сборку LSP-сервера [[https://www.eclipse.org/downloads/download.php?file=/jdtls/snapshots/jdt-language-server-1.49.0-202507102148.tar.gz][отсюда]]:

#+begin_src emacs-lisp :tangle yes
(setq my/jdt-language-server-dir (expand-file-name
                                  (file-name-concat user-emacs-directory
                                                    ".cache"
                                                    "lsp"
                                                    "jdt-language-server")))

(unless (file-exists-p my/jdt-language-server-dir)
    (let ((jdt-file-archive (file-name-concat my/config-dir
                                              "lsp-saves"
                                              "jdt-language-server-1.49.0.tar.gz")))
        (make-directory my/jdt-language-server-dir t)
        (let ((default-directory my/jdt-language-server-dir))
            (shell-command (concat "tar -xzf " jdt-file-archive)))))
#+end_src

Функция ниже возвращает команду запуска LSP-сервера. Сервер качнул
[[https://www.eclipse.org/downloads/download.php?file=/jdtls/snapshots/jdt-language-server-1.49.0-202507102148.tar.gz][отсюда]], согласно [[https://github.com/eclipse-jdtls/eclipse.jdt.ls][инструкции]] (из [[https://github.com/eclipse-jdtls/eclipse.jdt.ls][репозитория]] не собрался). Поддержку
Lombok облегчаем до минимума, иначе он вешает редактор. Для Spring
Boot накинем ресурсов.

#+begin_src emacs-lisp :tangle yes
(defun my/make-run-lsp-java-command ()
    (let* ((lsp-dir my/jdt-language-server-dir)
           ...
           ;;                              "lsp-data")))
           (config-dir (file-name-concat lsp-dir
                                         "config_linux")))
        (list "python"
              (file-name-concat lsp-dir "bin" "jdtls")
              ;; Linux configuration:
              "-configuration" (file-name-concat lsp-dir "config_linux")
              ;; For Lombok:
              (concat "--jvm-arg=-javaagent:" my/java-lombok)
              ;; "--jvm-arg=-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=1044"
              "--jvm-arg=-Dlombok.disableConfig=true"
              "--jvm-arg=-Dlombok.anyConstructor.suppressConstructorProperties=false"
              "--jvm-arg=-Dlombok.addLombokGeneratedAnnotation=true"
              "--jvm-arg=-Dlombok.extern.findbugs.addSuppressFBWarnings=false"
              ;; Increase resources for Spring Boot:
              "--jvm-arg=-Xmx4G"  ;; "--jvm-arg=-Xmx2G"
              ;; "--jvm-arg=-Xms1G"
              "--jvm-arg=-XX:+UseG1GC"
...
Регистрируем LSP-клиента с высшим (единица) приоритетом.
...
(lsp-register-client
 (make-lsp-client
  :new-connection (lsp-stdio-connection #'my/make-run-lsp-java-command)
  :activation-fn (lsp-activate-on "java")
  :priority 1
  :server-id 'myjdtls))
#+end_src

*** COMMENT Регистрируем новый LSP-сервер (java launch)

/Здесь полностью рабочий вариант. Чего не смог от него добиться - это/
/перехода на код определений из сторонних библиотек./

/JDTLS запускаем напрямую через Java./
...
Аналогично сохраняем сборку LSP-сервера [[https://www.eclipse.org/downloads/download.php?file=/jdtls/snapshots/jdt-language-server-1.49.0-202507102148.tar.gz][отсюда]]:
...
(defun my/make-run-lsp-java-command ()
    (let* ((lsp-dir my/jdt-language-server-dir)
           (lsp-jar (expand-file-name
                     (file-name-concat lsp-dir
                                       "plugins"
        ...
        (list "java"
              ...
              (concat "-javaagent:" my/java-lombok)
              "-Dlombok.disableConfig=true"
              "-Dlombok.anyConstructor.suppressConstructorProperties=false"
              "-Dlombok.addLombokGeneratedAnnotation=true"
              "-Dlombok.extern.findbugs.addSuppressFBWarnings=false"
              "-Xmx4G"
              "-XX:+UseG1GC"
...
*** COMMENT Eglot попытка (java launch)

Готовимся аналогично настроить eglot.
...
Сделаем генерацию путей к директориям data, workspace и cache.

#+begin_src emacs-lisp :tangle yes
(defun my/java-lsp--get-workspace-dir ()
    (expand-file-name
     (file-name-concat user-emacs-directory
                       "workspace"))
    ;; (my/java-lsp--get-data-dir)
    )

(defun my/java-lsp--get-cache-dir ()
    (expand-file-name
     (file-name-concat (my/java-lsp--get-workspace-dir)
                       ".cache")))

(defun my/java-lsp--get-data-dir ()
    (expand-file-name
     (file-name-concat user-emacs-directory
                       "workspace"
                       "lsp-data"
    ...
    (let* ((lsp-dir my/jdt-language-server-dir)
           (lsp-jar (expand-file-name
                     (file-name-concat lsp-dir
                                       "plugins"
                                       "org.eclipse.equinox.launcher_1.7.0.v20250519-0528.jar")))
           (java-library-dir (expand-file-name
                              (file-name-concat (my/java-lsp--get-workspace-dir)
                                                ".javalibs")))
           (workspace-dir (my/java-lsp--get-workspace-dir))
           (cache-dir (my/java-lsp--get-cache-dir))
           (data-dir (my/java-lsp--get-data-dir))
           (config-dir (file-name-concat lsp-dir
                                         "config_linux"))
           )
        (mapc (lambda (dir)
        ...
        (list "java"
              ...
              ;; "-Djava.import.maven.enabled=true"
              ;; "-Djava.import.gradle.offline.enabled=false"
              ;; "-Djava.import.gradle.wrapper.enabled=true"
              ;; "-Djava.import.gradle.version=current"

              ;; "-Djava.import.downloadSources=true"
              ;; "-Djava.import.autoAttachSources=true"
              ;; "-Djava.import.downloadJavadoc=true"
              ;; "-Dmaven.downloadSources=true"

              ;; "-Dorg.eclipse.core.resources.compatibility=true"
              ;; "-Dorg.eclipse.core.runtime.compatibility.override=false"
              ;; "-Dorg.eclipse.core.internal.resources.Compatibility.ignorePreviousSession=true"
              ...
              "-Dlombok.extern.findbugs.addSuppressFBWarnings=false"
              "-jar" lsp-jar
              ;; (concat "-Djava.import.common.workspace=" workspace-dir)
              ;; (concat "-Djava.import.common.workspace.cache=" cache-dir)
              "-configuration" config-dir
              "-data" data-dir
              "--add-modules=ALL-SYSTEM"
              "--add-opens" "java.base/java.util=ALL-UNNAMED"
...
(add-to-list 'eglot-server-programs
      '((java-ts-mode java-mode) . my/eglot-java--run))

(defun my/eglot-java--run (is-interactive)
    (interactive)
    (append (my/make-run-lsp-java-command)
...
*** Остальные настройки под lsp-java

Автоматический запуск LSP-сервера при открытии java-файлов.

При запуске JDTLS через jsp-java нужно помнить:
- По-умолчанию lsp-mode + lsp-java будут сваливать все проекты в одну
  директорию workspace, где они легко могут начать конфликтовать.
- В сыром запуске JDTLS предусмотрен параметр =-data=, в котором
  задается директория под файлы проекта. В lsp-java путь к этой
  директории лежит в переменной =lsp-java-workspace-dir=. Вот эту
  переменную и надо сделать уникальной для каждого из проектов.
- В аналогичной переменно =lsp-java-workspace-cache-dir= модуль
  jsp-java научился складывать исходники сторонних библиотек, что
  очень важно при работе с java. Ведет себя этот механизм очень
  капризно, поэтому эту переменную никак не трогаем.
- И еще важно для каждого java-проекта запускать свой инстанс JDTLS. В
  lsp-mode этого можно добиться через вызов функции =lsp= с
  параметром =t=.
Все это делается ниже. Подготовим сперва функцию создания пути к
директории workspace под каждый проект.
...
JDTLS требует много ресурсов и быстро вешает редактор. Поэтому
облегчаем работу сервера через выключение части функций.

Однако, этого ничего не нужно, если работаем не с Java'ой. Поэтому
снизим нагрузку на jdtls в буфере перед его запуском.

Все это упаковываем в локальные переменные java-буферов.

#+begin_src emacs-lisp :tangle yes
(setq my/java-hooks '(java-mode-hook
                      java-ts-mode-hook))

(defun my/run-java-lsp ()
    (setq-local lsp-modeline-diagnostics-enable nil  ;; Too hard for jdtls.
                lsp-modeline-code-actions-enable nil
                lsp-modeline-workspace-status-enable nil
                lsp-java-progress-reports-enabled nil
                lsp-ui-sideline-enable nil
                lsp-java-workspace-dir (my/java-lsp--get-data-dir)  ;; Unique data.
                )
    (lsp t))  ;; Different instance for each project.

(dolist (hook my/java-hooks)
    (add-hook hook 'my/run-java-lsp))
#+end_src
...
Подгрузка источников сторонних библиотек в проекте.
...
Позаботимся о выполнении org-babel блоков.

#+begin_src emacs-lisp :tangle yes
(my/--register-org-babel 'java)
#+end_src

Помогаем заполнять поля serialVersionUID. В коде это поле делают в
виде отрицательного числа. Для этого просто вернем случайное число из
L-диапазона (0 - 1000000000000000000).

/Для справки привожу настоящий L-диапазон:/
- Минимальное  значение: -9 223 372 036 854 775 808 (=Long.MIN_VALUE=)
- Максимальное значение: +9 223 372 036 854 775 807 (=Long.MAX_VALUE=)

Генерацию положим в сниппет для prog-mode:
: Serial version
: _sver

*** Отладка (dap-java)

Для Java надо затребовать модуль dap-java. Остальное настроено в
dap-mode.
...
Одна из важных вещей - это подтянуть импорт для какой-либо сущности в
коде. Повесим это действие (=lsp-java-add-import=) на клавиши. Заодно
и приплюсуем еще пару похожих функций.

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'lsp-mode-map
               "c j" '(:ignore t :which-key "java")
               "c j i" '(:ignore t :which-key "add/insert")
               "c j i i" '(lsp-java-add-import :which-key "Import")
               "c j i t" '(lsp-java-add-throws :which-key "Throws")
               "c j i u" '(lsp-java-add-unimplemented-methods
                           :which-key "Unimplemented methods"))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
               "c j d" '(:ignore t :which-key "debug")
               "c j d d" '(dap-java-debug :which-key "Debug")
               "c j d c" '(dap-java-debug-test-class
                           :which-key "Debug test class")
               "c j d m" '(dap-java-debug-test-method
                           :which-key "Debug test method"))
#+end_src

*** Информация на память

Примеры закачки пакетов через maven:
...
: mvn dependency:get -Dartifact=org.springframework.boot:spring-boot-starter-test:3.2.0
...
и для совместимости с отстающими ts-ls (2026-08-19):
: sudo npm install -g typescript@5.7.3
...
(defun my/--lsp-except-json ()
    (unless (member major-mode '(json-mode json-ts-mode))
        (lsp)))
#+end_src
...
(use-package perl-ts-mode
    :ensure t)
;; (lsp-ensure-server 'perlnavigator)
(fset 'perl-mode 'cperl-mode)
#+end_src

Устанавливаем LSP-сервер PLS, как показано в
- [[https://emacs-lsp.github.io/lsp-mode/page/lsp-pls/][LSP Mode - LSP support for Emacs / PLS]]
но под =sudo=:
: which cpan
: sudo /usr/bin/core_perl/cpan PLS

#+begin_src emacs-lisp :tangle yes
(use-package lsp-mode
    :hook (((perl-mode cperl-mode) . lsp-deferred)))
#+end_src

Позаботимся о вычислении org-babel блоков.

#+begin_src emacs-lisp :tangle yes
(my/--register-org-babel 'perl)
#+end_src
...
(my/treesit-install-language-grammar
 'perl
 '("https://github.com/tree-sitter-perl/tree-sitter-perl" "release"))
#+end_src
...
** Запрещаем временно ts-режимы (treesit поломался)

Treesit как-то поломался - написал костыль - далее treesit
починили. Но не пропадать же костылю, а вдруг пригодится еще раз.

Костыль на старте редактора вычищает из настроек все
ts-режимы. Сработает, если в t установлена переменная
=my/disable-ts-mode= (настраивается через customize).

#+begin_src emacs-lisp :tangle yes
(defcustom my/disable-ts-mode nil
  "Disable treesit (*-ts-mode) if t."
  :type 'boolean)
#+end_src

Функция, которая убирает из настроек ts-режимы.

#+begin_src emacs-lisp :tangle yes
(defun my/disable-ts-modes ()
    (my/ts--remove-from-major-mode-remap-alist)
    (my/ts--replace-in-auto-mode-alist))

(defun my/ts--remove-from-major-mode-remap-alist ()
    (setq major-mode-remap-alist
          (cl-remove-if (lambda (pair)
                            (let* ((mode (cdr pair))
                                   (mode-name (symbol-name mode)))
                                (string-search "-ts-" mode-name)))
                        major-mode-remap-alist)))

(defun my/ts--replace-in-auto-mode-alist ()
    (setq auto-mode-alist
          (mapcar (lambda (pair)
                      (if-let* ((file-ext (car pair))
                                (mode (cdr pair))
                                (is-symbol (symbolp mode))
                                (mode-name (symbol-name mode))
                                (is-ts-mode (string-search "-ts-mode" mode-name))
                                (mode-new-name (string-replace "-ts-mode"
                                                               "-mode"
                                                               mode-name)))
                          (cons file-ext (intern mode-new-name))
                          pair))
                  auto-mode-alist)))
#+end_src

Убираем ts-режимы, если это прописано в customize.

#+begin_src emacs-lisp :tangle yes
(when my/disable-ts-mode
    (my/disable-ts-modes))
#+end_src

** Возвращаем поведение по умолчанию для переходов по ссылкам в режиме редактирования

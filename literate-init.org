...
- [[#программирование][Программирование]]
  ...
  - [[#kotlin][Kotlin]]
  - [[#lua][Lua]]
  - [[#java-with-lsp-java][Java (with lsp-java)]]
...
** COMMENT Java (custom LSP-server)

/Здесь полностью рабочий вариант. Чего не смог от него добиться - это/
/перехода на код определений из сторонних библиотек./

Чтобы более-менее полноценно работать с Java в Emacs, нужно собрать
=последюю версию LSP-сервера=. Это важно! поскольку на момент
составления этого параграфа функция lsp-install-server устанавливала
что-то старое, несовместимое с текущими Java и Gradle-проектами.
...
*Примечание:* /Все нужные пакеты можно было скачать через maven, но/
/предпочел добавить их в этот репозиторий. Ниже для примера оставляю,/
...
(defun my/make-run-lsp-java-command ()
    (let* ((lsp-dir my/jdt-language-server-dir)
        ...
        (list "python"
              ...
              ;; Data directory:
              ;; "-data" data-dir  ;; Too strange work.
              )))
#+end_src
...
/Этот раздел закомментирован, поскольку все равно пока не работает./

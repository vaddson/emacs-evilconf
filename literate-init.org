...
Aider, Pi или Claude, но нативный для Emacs. Контекстное окно Macher
держит в org-буфере, дополняя его по мере необходимости. Заголовки в
этом буфере - вопросы и задания. При необходимости что-то изменить в
...
(defun my/git-apply-buffer ()
    ...
    (if (= emacs-major-version 30)
        (let ((default-directory (my/macher-get-default-directory)))
            (if (my/get-git-apply-region-result)
...
(defun my/get-git-apply-region-result ()
    (zerop

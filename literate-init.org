...
(defun my/llm-codework-new-theme (lang)
    (interactive (list (read-string "Language: "
                                    (my/get-lang))))
    (let* ((prompt (format "Remember codes in %s (if exists). Codes starts with the <src_code_block> tag and ends with the </src_code_block> tag. All explanations give in russian." lang)))
        (my/llm-codework-switch-to-out-buffer my/llm-codework-out-buffer)
        (goto-char (point-max))
        (unless (boundp 'my/header-count)
            (setq-local my/header-count 0))

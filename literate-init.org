...
динамических блоках:
#+begin_src emacs-lisp :tangle no :exports results :hidden
(mapcar (lambda (pair)
            `(,(symbol-name (car pair))))
        my/org-roam--sort-fn-alist)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
;; (defun my/org-roam--sort-title (node1 node2)
;;     (string< (org-roam-node-title node1)
;;              (org-roam-node-title node2)))

;; (org-roam-ql-register-sort-fn 'title 'my/org-roam--sort-title)

(setq my/org-roam--sort-fn-alist
      '((title
         . (lambda (node1 node2)
               (string< (org-roam-node-title node1)
                        (org-roam-node-title node2))))
        ...
        (file-mtime
         . (lambda (node1 node2)
               (string< (format-time-string "%Y-%m-%dT%H:%M:%S"
                                            (org-roam-node-file-mtime node1))
                        (format-time-string "%Y-%m-%dT%H:%M:%S"
                                            (org-roam-node-file-mtime node2)))))
        (file-mtime-r
         . (lambda (node1 node2)
               (not (string< (format-time-string "%Y-%m-%dT%H:%M:%S"
                                                 (org-roam-node-file-mtime node1))
                             (format-time-string "%Y-%m-%dT%H:%M:%S"
                                                 (org-roam-node-file-mtime node2))))))
        (file-atime
         . (lambda (node1 node2)
               (string< (format-time-string "%Y-%m-%dT%H:%M:%S"
                                            (org-roam-node-file-atime node1))
                        (format-time-string "%Y-%m-%dT%H:%M:%S"
...
(dolist (pair my/org-roam--sort-fn-alist)
    (org-roam-ql-register-sort-fn (car pair) (cdr pair)))
#+end_src
...
LSP-сервер для Scala называется [[https://scalameta.org/metals/][Metals]]. На Arch его можно установить
из AUR-пакета:
: yay -S metals

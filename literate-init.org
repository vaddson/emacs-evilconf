...
(defun my/lsp-restart ()
    (interactive)
    (if (boundp 'lsp--session)
        (progn (setq lsp--session nil)
               ;; (call-interactively 'lsp-workspace-restart)
               (lsp-shutdown-workspace)
               (lsp))
        (message "WARNING: there is no lsp--session variable!")))
#+end_src
...
(my/leader-def nil 'override
               ...
               "c l s" '(lsp-shutdown-workspace :which-key "Shutdown LSP workspace")

...
(defun my/key-to-header (header-key)
    "Convert keyword like :CONTENT-TYPE to string like \"Contet-type\"."
...
#+begin_src emacs-lisp :tangle yes
(advice-add #'ws-parse :around
            (lambda (func proc str &rest args)
...
(defun my/giga--proxy-handler (request)
    (if-let* ((process (ws-process request))
        ...
        (let* ((url-request-data (ws-body request))
               (url-request-method (my/ws-method-str request))
               (url-request-extra-headers (my/giga--insert-key-to-headers
                                           (my/giga--correct-contet-length
                                            url-request-data
                                            headers-in)))
               (response-buffer (url-retrieve-synchronously
                                 (my/giga--make-url request))))
            (with-current-buffer response-buffer
...
(defun my/giga--make-url (request)
    (string-join (cl-remove-if 'not
                               (list (concat "https://gigachat.devices.sberbank.ru"
                                             (my/ws-uri request))

...
Иногдв нужно для отладки кода в этом документе.
...
Есть неприятность при создании коммита через Magit: почему-то тоже
поднимается server-after-make-frame-hook.  Понять, что это "был Magit"
можно по имени текущего буфера - =" *server*"= (определил
экспериментально).
...
Проект бывает удобно открыть сразу вместе с созданием одноименного
рабочего пространства и назначением заголовка фрейма. Сделаем такую
возможность.

#+begin_src emacs-lisp :tangle yes
(defun my/switch-project-with-workspace ()
    (interactive)

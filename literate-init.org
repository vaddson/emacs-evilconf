...
В параграфе [[id:6435cae0-700a-4263-966a-cbebc8f9ae25][C++ (default LSP-server)]] уже есть настройки для
подключения к clangd на удаленной машине. Здесь покажем, как закинуть
clangd на эту удаленную машину, если нет возможности установить его
напрямую из репозитория.

Решением тут может быть clangd в докере [[https://hub.docker.com/r/lspcontainers/clangd-language-server][lspcontainers/clangd-language-server]].

...
- Делаю редактор с Vim-раскладкой для редактирования.
  ...
  функции =my/leader-der=, =my/localleader-def= и =my/define-key=.
- Практически вся необходимая мне функциональность из Doom восстановлена.
  Можно отправляться в свободное плаванье.
...
В комментарии оставлено все, что там есть.
...
(setq my/localleader "SPC m"
      my/localleader-non-normal "M-m m")
#+end_src

Для определения клавиш лучше переключиться на пакет =general=.
Для некоторых режимов отменяем клавишу пробела.
...
#+begin_src emacs-lisp :tangle yes
(defun my/leader-def (states keymaps &rest args)
    (apply 'my/--define-key
           my/leader
...
(defun my/localleader-def (states keymaps &rest args)
    (apply 'my/--define-key
           my/localleader
...
(defun my/define-key (states keymaps &rest args)
...
(defun my/--define-key (leader leader-non-normal states keymaps &rest args)
    (setq states (or states
                     '(normal visual insert replace moniton emacs)))
    (apply 'general-define-key
           :states states
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override ";" 'pp-eval-expression)
#+end_src

** Настройка поведения клавиш для evil-mode, как в Vim.

Перемещение по строкам и внутри них хоть и сделано в evil-mode, но
работает как-то все равно с огрехами. Поэтому сделаем костыль, в
котором четко определим, как должна работать навигация.
...
(my/leader-def nil 'override "e d" #'my/correct-font-in-current-frame)
#+end_src
...
может быть функция из программного кода или заголовок в org-mode.
...
Если размещать работу по разным Emacs'ам, то скоро можно запутаться, в
каком из них что.  Здесь спасает текст в заголовке окна
(фрейма). Нужно иметь возможность назначать текст окна интерактивно.
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
    "n i" #'my/set-title-bar-interactive)
#+end_src
...
(pcase (daemonp)
    ...
    ("rest"
     (setq my/transparency_level 1)
     (load-theme 'doom-one t)))
#+end_src

А тема по-умолчанию задается на [[id:40986685-fc80-45b7-ab77-4cac074fea9f][старте]].

Клиентов к демонам запускаю по ярлыкам из [[/usr/share/applications/]].
Если при вызове клиента демон не был запущен, то он запускается.
...
Отдельно сделаем показ имени последней перспективы.

#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
    "TAB v" #'my/show-last-perspective)
...
(defun my/prepare-frame-for-workspace (persp-name frame)
    (select-frame-set-input-focus frame)
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
    "o f" #'my/make-frame-and-switch-perpective
    "q f" #'delete-frame
    "n I" #'my/set-title-bar-and-workspace-interactive)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
    "w 1" #'spacemacs/window-split-single-column
    "w 2" #'spacemacs/window-split-double-columns
    "w 3" #'spacemacs/window-split-triple-columns
...
(my/leader-def nil 'override
    ...
    "w V" #'my/split-window-right-and-focus
    "w s" #'evil-window-split
    "w S" #'my/split-window-below-and-focus)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
    "b j" #'projectile-switch-to-buffer-other-window
    "b J" #'counsel-switch-buffer-other-window)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
    "b 1" #'move-buffer-window-no-follow-1
    "b 2" #'move-buffer-window-no-follow-2
    "b 3" #'move-buffer-window-no-follow-3
    "b 4" #'move-buffer-window-no-follow-4
    "b 5" #'move-buffer-window-no-follow-5
    "b 6" #'move-buffer-window-no-follow-6
    "b 7" #'move-buffer-window-no-follow-7
    "b 8" #'move-buffer-window-no-follow-8
    "b 9" #'move-buffer-window-no-follow-9)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
    "f i" #'spacemacs/find-file-vsplit
    "f n" #'spacemacs/find-file-split)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(my/define-key '(normal visual motion) 'org-mode-map
...
#+begin_src emacs-lisp :tangle yes
(my/define-key '(normal visual motion) 'evil-collection-unimpaired-mode-map
	"[ l" nil
	"] l" nil)
#+end_src
...
Для наглядности бывает нужно схлопнуть ширину какого-либо столбца в
таблице. Проще, конечно, отключить перенос строк функцией
=toggle-truncate-lines=.
...
(my/define-key 'normal 'treemacs-mode-map
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
    "s g" #'sdcv-search-input
    "s w" #'sdcv-search-pointer)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
    "o c" #'my/calculator-python-toggle)
#+end_src
...
(my/define-key 'normal 'prog-mode-map
    "RET" #'lsp-ui-doc-show
    "K" #'my/eldoc)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(my/define-key 'insert 'plantuml-mode-map
...
(my/define-key 'normal 'plantuml-mode-map

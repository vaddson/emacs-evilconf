...
- [[#llm-всевозможные-клиенты][LLM. Всевозможные клиенты]]
  ...
  - [[#gptel-подключаем-gigachat][GPTel. Подключаем GigaChat]]
    ...
    - [[#aider-в-связке-с-gigachatом-только-информация][Aider в связке с GigaChat'ом (только информация)]]
...
(defun my/giga--proxy-handler (request)
    (if-let* ((process (ws-process request))
        ...
        (let* ((url-request-data (my/giga--request-data request))
               ...
               ;; ;; !!!
               ;; (lala (my/log (my/giga--debug-request-to-string url-request-method
               ;;                                                 url
               ;;                                                 url-request-extra-headers
               ;;                                                 url-request-data)))
               ;; (lala (my/log "\n"))
...
#+RESULTS:
: my/giga--debug-headers-to-string

*** Aider в связке с GigaChat'ом (только информация)

- Подготовка ::

  1. Установить aider.

  2. Положить настройки для GigaChat в файл:

...
#+begin_src emacs-lisp :tangle yes
(defun my/llm-codework-new-theme ()
    (interactive)
    (let* ((lang (my/get-lang))
           (prompt (format "Remember codes in %s (if exists). Codes starts with the <src_code_block> tag and ends with the </src_code_block> tag. All explanations give in russian." lang)))
        (my/llm-codework-switch-to-out-buffer my/llm-codework-out-buffer)
        (goto-char (point-max))
        (unless (boundp 'my/header-count)
            (setq-local my/header-count 0))
...
Пока будем пользоваться простеньким =perlnavigator=, поскольку по
инструкции отсюда: [[https://emacs-lsp.github.io/lsp-mode/page/lsp-perl/][LSP Mode - LSP support for Emacs / Perl]] установить
сервер пока не получилось.

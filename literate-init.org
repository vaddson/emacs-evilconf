...
(unless (boundp 'is-nerd-icons-install-fonts)
    ;; TODO: Перенести custom в отдельный файл и сделать загрузку в начале.
    ;;       А пока будем при каждом запуске спрашивать установку:
    (unless (daemonp)
...
Уберем неудобное удаление слова перед курсором по Ctrl+Backspace.
...
/Пока не понял, почему вызванный неинтеракривно ediff-buffer всегда/
/показывает одну разницу во весь файл./
...
(defun my/clang-format-ediff-buffers ()
    ...
    (when (member major-mode '(c-mode c++-mode c-ts-mode c++-ts-mode))
        (let ((buffer (current-buffer)))
            (delete-other-windows)
            (async-shell-command (concat "clang-format --style=file "
            ...
            (let ((out-buffer (current-buffer)))
                (evil-window-left 1)
                ;; (ediff-buffers buffer out-buffer)  ;; Don't work. Why?!
